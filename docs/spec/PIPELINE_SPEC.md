...
- collect pair recovery prompt содержит command-first absolute heredoc write targets для suggested authored doc + task-specific manifest skeleton; pair-recovery heredoc тоже является marker-bearing bootstrap surface и не validation-acceptable до targeted evidence pass. Provider обязан после heredoc удалить `ACP_COLLECT_BOOTSTRAP_REPLACE_BEFORE_EXIT`, заменить Recovery Bootstrap/Recovery Summary scaffold на concrete repo evidence и переписать manifest; marker-free low-signal recovery summaries reject-ятся тем же collect validation, чтобы short valid-artifact stop не закреплял recovery-only docs. Manifest-only repair prompt начинается с `FIRST COLLECT MANIFEST REPAIR COMMAND` и сразу даёт absolute heredoc write target с task-specific JSON skeleton, overwrite-only instruction для existing invalid manifests и compact validation checklist; authored docs/evidence уже encoded в skeleton, root-file repair evidence использует тот же `README` -> `Makefile` -> build/deploy preference, а validation-error details не повторяются как field-level patch cues. Это не ACP-side manifest synthesis, а provider instruction surface под write-set guard.
