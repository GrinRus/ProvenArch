...
func CollectManifestContractLines(artifactRoot string) []string {
	lines := []string{
		...
		`- Each citations[] item MUST include: id, repo, path, non-empty claim_ids, and document_ids.`,
		...
		`- Do NOT add top-level claims, claim_map, validation, metadata, compatibility, or alternate semantic wrapper fields; claim IDs live only in citations[].claim_ids.`,
		`- Do NOT leave template placeholders inside claim_ids or JSON strings; literal tokens such as SHARD, <shard>, <claim>, TODO, or REPLACE_ME make the manifest invalid.`,
		...
		`- semantic.coverage.notes MUST be an array of strings; a single string value is invalid.`,
		`- semantic.questions[*] MUST include id and text; do NOT omit stable question ids.`,
		`- semantic.entities[*] MUST remain full entity objects with id, name, type, and provenance; do not drop provenance during repair or retry flows.`,
		`- semantic.entities[*] MUST NOT put repo/path/evidence directly on the entity; repository evidence belongs under provenance.evidence[].`,
		`- semantic.edges[*] MUST use id, type, from, to, and provenance; do NOT use relation/source/target aliases.`,
		`- semantic.findings[*] MUST remain structured finding objects; never collapse findings to strings.`,
		`- semantic.findings[*] MUST include id, severity, title, description, and provenance; severity must be one of the canonical schema values.`,
		`- semantic provenance MUST include kind, numeric confidence, and evidence[]; do NOT use direct {"repo":"...","path":"..."} provenance objects.`,
		...
		`- Authored collect markdown MUST be operator-facing architecture evidence, not runtime process narration: do not mention bounded reads/passes, guessed paths/files/evidence, expected-missing path checks, recovery attempts, or later repair as final content.`,
...
func CollectManifestLegacyHygieneLines() []string {
	return []string{
		...
		`- semantic.coverage.notes MUST be an array, not a string.`,
		`- semantic.questions[*] MUST use id + text; do NOT use question or other alias keys.`,
		`- semantic.entities[*] MUST use id + name + type + provenance; do NOT use kind, repo, path, or evidence as entity-level aliases.`,
		...
		`- semantic provenance objects MUST include kind + confidence + evidence[]; do NOT use provenance as a direct repo/path object.`,
		`- provenance.confidence values MUST stay numeric; string confidence values are invalid.`,
		`- semantic.findings[*] MUST keep id/severity/title/description/provenance and evidence inside provenance.evidence; do NOT use evidence_citation_ids or legacy inference/summary compatibility fields.`,
...
func CompactCollectManifestValidationChecklist(artifactRoot string) []string {
	return []string{
		...
		`- citations[] items use id, repo, path, non-empty claim_ids, document_ids; citations[].id values must be unique, and every documents[].citation_ids value references a citations[].id.`,
		`- when citing multiple repo files from one authored markdown document, derive each citations[].id from the shard/document stem plus the repo path slug, not from the authored document id alone.`,
		`- do not add top-level claims/claim_map/metadata/validation/compatibility; claim IDs belong only under citations[].claim_ids.`,
		`- do not leave template placeholders such as SHARD, <shard>, <claim>, TODO, or REPLACE_ME in claim_ids or JSON strings.`,
		...
		`- authored collect markdown must not mention bounded reads/passes, guessed paths/files/evidence, expected-missing path checks, recovery attempts, or later repair as final content.`,
		...
		`- semantic.coverage.notes must be an array of strings, not a single string.`,
		`- semantic.entities[] items must use id/name/type/provenance; do not put repo/path/evidence directly on entities.`,
		`- semantic.edges[] items must use id/type/from/to/provenance; relation/source/target are forbidden aliases.`,
		`- semantic.findings[] items, when present, must include id, severity, title, description, and provenance.`,
		`- semantic provenance objects must include kind, numeric confidence, and evidence[]; provenance as direct repo/path is invalid.`,
		...
		`- forbidden legacy aliases: step_contract, compatibility, covered_topics, question, relation, source, target, entity-level repo/path/evidence, direct repo/path provenance, evidence_citation_ids, string confidence, summary as finding alias.`,
	}
}
...
func ValidatorVerdictContractLines() []string {
	return []string{
		...
		`- Provider-authored fixed_paths MUST be []; only the orchestrator may add repaired paths after deterministic revalidation.`,
		`- verdict=PASS MUST NOT contain an error-severity issue; an effective verdict=FAIL MUST contain at least one technical error issue.`,
		`- issues[] MUST be unique and emitted in deterministic code/path/document_id/citation_id order.`,
