...
- для `init.step1.collect` и `refresh.step1.collect` collect prompt включает suggested authored doc path и literal task-specific manifest skeleton, чтобы provider писал `shard-pack-manifest.json` сразу после первого authored doc. Общий runtime делает не более одной manifest-only repair попытки, если provider уже записал authored docs, но `write_root/shard-pack-manifest.json` missing/invalid; repair prompt включает task-specific scaffold (authored docs, `artifact_root`, repo/path scopes, candidate evidence paths и literal JSON skeleton), а engine write-set guard разрешает менять только `write_root/shard-pack-manifest.json`.
