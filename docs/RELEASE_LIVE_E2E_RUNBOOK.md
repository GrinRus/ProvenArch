...
- `reports/taskruns/<run_id>-quality.json` обязан содержать fresh `artifact_inventory` для текущего run: promoted workspace surfaces, taskrun/staging surfaces и semantic counts из текущего `final-run-index.json`. `run_warnings` с префиксом `artifact_quality:` считаются canonical live gate blocker даже при schema-valid `validator-verdict.json = PASS`; типовые blockers — refresh final set с несколькими canonical docs и единственным generic `cite.runtime-summary`, placeholder top-level reports/proposals, empty semantic model, missing `model/entities`, gap-only C4, empty findings при owner/operational/dependency coverage gaps и shard manifests, которые ссылаются на provider/tool side-effect paths.
