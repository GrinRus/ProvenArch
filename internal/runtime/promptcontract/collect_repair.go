...
func ComposeCollectArtifactPairRepairPrompt(provider acpruntime.Provider, task acpruntime.Task, validationErr error) string {
	...
	lines := []string{
		...
		"- The command writes a marker-bearing recovery bootstrap pair; it is not final-acceptable until you enrich it.",
		...
		"- After the command succeeds, do one targeted evidence pass using existing repo entrypoint hints and assigned path_scopes, then overwrite the exact authored document and shard-pack-manifest.json targets with evidence-backed content.",
		"- Successful recovery output must not contain ACP_COLLECT_BOOTSTRAP_REPLACE_BEFORE_EXIT, Recovery Bootstrap/Recovery Summary scaffold text, or the normal collect first-action scaffold prose.",
		"- Exiting successfully immediately after the heredoc command is invalid; an unchanged recovery bootstrap pair is an artifact_quality blocker.",
