...
- headless providers in MVP: `claude-code` (default), `qwen-code`, and `codex-code`
- provider selection precedence: `--runtime-provider` > `ACP_RUNTIME_PROVIDER` > `claude-code`
- provider commands: `ACP_CLAUDE_CMD` (default `claude-code`), `ACP_QWEN_CMD` (default `qwen`), `ACP_CODEX_CMD` (default `codex`)
