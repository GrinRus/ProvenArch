...
5) **Runtime providers (`internal/runtime/*`)** *(implemented baseline)*
   ...
   - для `init.step1.collect` / `refresh.step1.collect` runtime выполняет максимум одну manifest-only repair попытку, если provider уже записал authored docs в `write_root`, но `shard-pack-manifest.json` отсутствует или невалиден; repair prompt и engine write-set guard разрешают менять только `write_root/shard-pack-manifest.json`
