...
- strict collect validation:
  ...
  - collect pair recovery запускается один раз при no authored artifacts + non-empty provider diagnostics или bootstrap-only authored docs, разрешает писать только suggested authored doc + `shard-pack-manifest.json`, а recovery bootstrap/scaffold должен быть заменён evidence-backed content до validation
