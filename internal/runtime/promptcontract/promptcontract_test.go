...
func TestComposeArtifactOnlyPromptUsesBoundedQwenToolFirstCollectContract(t *testing.T) {
	...
	for _, token := range []string{
		...
		"validator-rejected phrases such as bounded read, bounded evidence read, bounded collection, bounded pass, guessed path/file/evidence, or missing expected evidence",
		...
		"every semantic provenance.kind must be exactly observation",
		"Do not use inference, assertion, observed, inferred, asserted, document, report, source, or any other value",
		"omit unsupported claims and record an observed coverage gap instead",
...
func TestComposeValidatorVerdictRepairPromptIsVerdictOnly(t *testing.T) {
	...
	expectedTokens := []string{
		...
		"never a repository evidence path",
...
func TestComposeDraftArtifactRepairPromptNamesExactTargets(t *testing.T) {
	...
	expectedTokens := []string{
		...
		"This structural recovery invocation has exactly one provider action",
		"Do not call read_file, glob, or another read-only tool first",
		...
		"This repair invocation is structural only: do not replace the bootstrap scaffold",
...
func TestComposeDraftArtifactEnrichmentPromptAvoidsBootstrapHeredoc(t *testing.T) {
	...
	for _, token := range []string{
		...
		"The first provider action must be one shell/terminal command that both reads bounded evidence and writes every referenced markdown target",
		"Do not call read_file, glob, or another read-only tool before that command",
		...
		"Never place a physical newline inside a quoted Python string",
		"Shell assignments such as MANIFEST=/path are not exported to child processes",
		"pass it as a positional argument",
		"never use os.environ[...] with a bare, unexported NAME=/path assignment",
		...
		"Never write repository-root shorthand such as repo:. or repo:./, and never write glob or wildcard syntax such as repo:src/*",
		...
		"Put each required Architecture Home heading on its own Markdown H2 line",
...
func TestComposeDraftArtifactEnrichmentPromptNamesShardStatusEvidenceAndTargetIdentity(t *testing.T) {
	...
	for _, token := range []string{
		...
		"Current target identity comes only from repo_scope/repo_scopes/domain_id",
		"never from ambient orchestration labels, workspace paths, or run-folder names",
		"final markdown must not name unrelated targets or repositories unless an allowed staged artifact",
		...
		"write exact current-run counts in the literal key=value shape planned=<n> succeeded=<n> failed=<n> incomplete=<n>",
		"planned=16 succeeded=16 failed=0 incomplete=0",
...
func TestComposeDraftArtifactEnrichmentPromptForProposalsRequiresWriteFirstTargets(t *testing.T) {
	...
	for _, token := range []string{
		...
		"Validation errors are diagnostic input only",
		"verify the exact current-run evidence file with test -f",
		"never allow an unset variable to produce text such as 'from  and'",
	} {
		if !strings.Contains(prompt, token) {
...
func TestComposeDraftArtifactEnrichmentPromptStep2CommandTextRetryRequiresTypedShardCompleteness(t *testing.T) {
	...
	for _, token := range []string{
		...
		"summary.md and architect-summary.md must include the exact key=value completeness literal planned=<n> succeeded=<n> failed=<n> incomplete=<n>",
...
func TestComposeArtifactOnlyPromptAddsAsIsDraftCanonicalSection(t *testing.T) {
	...
	expectedTokens := []string{
		...
		"FIRST AS-IS DRAFT COMMAND:",
		"one bounded evidence-read/write action sequence",
		"first filesystem action may perform one bounded current-run evidence read/list",
		"Run at most one bounded current-run evidence read/list command as the next action",
		"immediately run one mechanically simple direct-literal write command",
		...
		"mkdir -p '/tmp/write-root' '/tmp/draft-root'",
		...
		`test -s '/tmp/draft-root/overview.md'`,
		`test -s '/tmp/draft-root/summary.md'`,
		`test -s '/tmp/draft-root/architect-summary.md'`,
		`test -s '/tmp/write-root/asis-draft-manifest.json'`,
		...
		"Before writing the manifest, scan all three completed markdown files case-insensitively for runtime/recovery markers",
		"Only after that three-file marker scan passes, write '/tmp/write-root/asis-draft-manifest.json' last",
