...
- `scripts/full-run-batch.sh` — canonical live batch + frontend live e2e:
  ...
  - frontend smoke работает на отдельной `frontend-workspace` копии backend workspace, merge-ит выбранный run snapshot reports и сохраняет product-authored Task/Attempt history поверх copied reports tree и не мутирует backend baseline; harness не создает и не переписывает product Task/Attempt identity, а Playwright выбирает ровно один completed public Task/Attempt по exact snapshot run вместо starting a second provider-backed init; missing/ambiguous identity is a fail-closed frontend evidence error; `snapshot_reports_missing` после terminal backend failure записывается как dependent skipped frontend status, а не independent frontend regression; shell tests закрепляют, что failed headless refresh row remains ineligible for frontend smoke even if a triage snapshot directory exists
