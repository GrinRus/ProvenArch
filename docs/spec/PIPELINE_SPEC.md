...
- collect prompt ставит suggested authored doc path и literal task-specific `shard-pack-manifest.json` skeleton в command-first section сразу после provider identity, чтобы provider писал marker-free seed collect pair сразу после первого authored shard doc, а не после broad repository sweep; до этого command provider не должен делать `read_file`, `list_directory`, `grep_search`, `glob` или repo exploration, а repo entrypoint hints в collect prompt применимы только после того, как first artifact pair уже существует. Эта heredoc command встречается один раз, а поздний docs-first block только ссылается на first-action skeleton; first-action pair является seed-only write surface: после command provider должен сделать один targeted evidence pass по entrypoint hints/path scopes и перезаписать authored doc + `shard-pack-manifest.json` более богатым evidence-backed content там, где repo evidence это поддерживает. Collect artifact monitor применяет extended enrichment window для collect post-/partial-artifact stalls перед repair; если provider stalls с unchanged seed pair, strict collect validation fail-ит shard вместо `artifact_only` success. Marker-bearing, marker-free unchanged seed и recovery-fallback collect docs считаются bootstrap-only/low-signal и блокируют hard pass.
- collect runtime до финальной validation может выполнить focused provider-authored recovery без ACP-side synthesis: если provider оставил stdout/stderr diagnostics, но не записал authored artifacts, общий engine делает одну collect pair-recovery попытку с write-set guard только на suggested authored doc + `write_root/shard-pack-manifest.json`; если provider уже записал bootstrap-only/seed-only authored doc, collect validation reject-ит такой referenced document и runtime не маскирует его pair-recovery success; если provider уже записал non-bootstrap authored docs, но `shard-pack-manifest.json` missing/invalid, общий engine делает одну manifest-only repair попытку с write-set guard только на `write_root/shard-pack-manifest.json`; ACP не нормализует `documents[].path`, не автозаполняет metadata и не использует semantic stdout repair
