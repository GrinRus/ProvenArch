...
- `reports/as-is/overview.md` is the canonical Architecture Home. Step2 validation requires non-empty `System at a glance`, `Analyzed scope`, `Domains and ownership`, `Key flows`, `Integrations and datastores`, `Where to start`, `Safe-change guidance`, and `Evidence gaps and open questions` sections. Concrete repo/path or canonical-document references are required where evidence exists; manifest recap, runtime/provider narration and unsupported confidence are not acceptable architecture content.
...
Before `refresh.step1.collect`, ACP writes `refresh-impact-plan.json` using `schemas/refresh-impact-plan.schema.json`. Git changes come from `git diff --name-status -z -M -C <baseline>..<current>` and preserve rename/copy identity. Planning accepts at most 10,000 complete changed paths; an exact larger count is recorded with `changes_complete=false`, no partial mapping, and `full_refresh_required`. Dirty/unavailable/diverged revisions, changed/unreadable analysis inputs, unreadable prior evidence and unmapped in-scope paths also require full refresh. The contract is `enforcement: advisory`: `unchanged_candidate` and `selective_candidate` do not skip or filter any current step; the orchestrator still executes the complete refresh pipeline.
