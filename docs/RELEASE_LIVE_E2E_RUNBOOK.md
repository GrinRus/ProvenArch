...
- для `init.step1.collect` и `refresh.step1.collect` collect prompt включает early pair-write requirement: suggested authored doc path + literal task-specific manifest skeleton должны быть записаны как focused artifact pair до broad second-pass repository sweep. Общий runtime делает не более одной manifest-only repair попытки, если provider уже записал authored docs, но `write_root/shard-pack-manifest.json` missing/invalid; repair prompt ставит фактические authored docs и exact JSON skeleton перед compact repair constraints, а engine write-set guard разрешает менять только `write_root/shard-pack-manifest.json`.
...
- Политика triage: если в `write_root` есть authored docs, общий engine должен сделать один manifest-only repair с task-specific scaffold; writes outside `shard-pack-manifest.json` или failure после repair остаются `runtime_contract_failed`.
