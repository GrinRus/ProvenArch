...
5) **Runtime providers (`internal/runtime/*`)** *(implemented baseline)*
   ...
   - `qwen-code` для `init.step1.collect` / `refresh.step1.collect` имеет internal stall watchdog в двух фазах:
     - `pre_artifact`: если process жив, но до первого authored artifact одновременно молчат stdout/stderr и мутации `write_root`, runtime принудительно завершает provider process и запускает один forced fresh retry; на второй попытке pre-artifact stall detector остаётся включённым, но получает расширенное bounded grace window вместо полного step-timeout budget; если и это окно выгорает, runtime пишет `retry exhausted`, и fully silent retry exhaustion (`stdout/stderr == empty`) классифицируется как `runner_unavailable`, а retry с provider output остаётся `runtime_contract_failed`
     - `post_artifact`: если в `write_root` уже есть `shard-pack-manifest.json` + authored docs, но provider перестал писать в stdout/stderr и перестал мутировать `write_root`, runtime принудительно завершает provider process, пишет diagnostic events (`runtime task stalled before artifacts` / `runtime task stalled after artifacts` / `retry scheduled` / `retry completed` / `retry exhausted`) и запускает forced artifact-repair retry до step timeout
   ...
   - `claude-code`, `qwen-code` и `codex-code` используют shared provider-agnostic step-policy/prompt layer для required artifacts, retry bans и explicit negative rules; shared validation/raw-diagnostics helpers живут в `internal/runtime/providercommon`, а provider-specific остаются command/process execution, qwen stall watchdog/retry semantics, transcript extraction и provider failure classification
   - runtime metadata для `fake`, `claude-code`, `qwen-code`, `codex-code` обязана иметь non-empty `meta.runtime.name` и `meta.runtime.version`; для headless providers version = `headless`, для fake = `fake`
