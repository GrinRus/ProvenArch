...
- strict collect validation:
  ...
  - manifest-only runtime repair запускается один раз только при authored docs + missing/invalid `shard-pack-manifest.json`; prompt requires reading existing authored docs and listed repo evidence candidates before the only manifest write, while the embedded skeleton remains schema guide only, and evidence-rich docs must produce concrete semantic entities/edges/findings beyond repo/shard wrappers and owner mapping
