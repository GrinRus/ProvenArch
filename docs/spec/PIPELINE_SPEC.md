...
- `qwen-code` draft steps (`step0.constitution`, `step2.asis_docs`, `step4.proposals`) могут остановиться после bounded valid-artifact window, если draft manifest и все referenced draft files уже валидны, но provider продолжает стримить или мутировать draft files; это artifact-only controlled completion, не расширение timeout, а collect/validator steps сохраняют обычную post-artifact stale-signal policy
