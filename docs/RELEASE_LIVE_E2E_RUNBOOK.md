...
- `init-inspect` обязан проверить readability выбранных Changes/Evidence artifacts через UI и `/api/artifacts`: markdown preview не должен быть placeholder/scaffold, C4 raw Mermaid не должен быть gap-only, Evidence/Publish preview panels должны иметь viewport-visible читаемую область, а screenshots должны подтверждать читаемый desktop/mobile Changes surface.
