...
Дополнительно:
- UI precheck timeout/зависание `npm ci` или `playwright install chromium` classify-ится как `precheck_failed`; проверять `precheck-ui-npm.log`/`precheck-playwright.log` на `[precheck-timeout]` и не продолжать deep runtime triage, если headless taskruns ещё не стартовали.
...
- child `full-run-batch.sh` обязан bounded-исполнять UI dependency/browser precheck; зависший `npm ci` или `playwright install chromium` не должен оставлять profile indefinitely running и обязан materialize-иться как `precheck_failed` с log evidence.
