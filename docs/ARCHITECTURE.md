...
3) **Orchestrator (`internal/orchestrator`)** *(implemented baseline)*
   ...
   - Collect recovery остаётся явной provider-authored поверхностью: non-silent no-artifact collect может выполнить одну pair-recovery попытку (`suggested overview doc + shard-pack-manifest.json`), а collect с non-bootstrap authored docs — одну manifest-only provider попытку; unchanged seed/bootstrap authored docs не маскируются repair success; write-set guards запрещают любые лишние файлы
