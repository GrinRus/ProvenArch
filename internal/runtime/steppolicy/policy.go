...
func DocFirstFilesystemPolicy(task acpruntime.Task) string {
	...
	case "init.step2.asis_docs", "refresh.step2.asis_docs":
		lines = append(lines,
			...
			`- Architecture Home must not mention run IDs, current-run analysis, pipeline step names, collect passes, typed shard plans/summaries, shard-pack manifests, completeness counters, repair/enrichment attempts, staged-source mechanics, or how runtime assembled the document; state repository facts and evidence gaps directly.`,
