...
- `init.step0.constitution`, `init|refresh.step2.asis_docs` и `init|refresh.step4.proposals` считаются successful только если runtime draft manifest валиден, все referenced draft files реально существуют под `draft_final_root`, а referenced markdown не остался unchanged first-action/recovery scaffold; для `step0` это значит, что `charter-overview.md` должен быть evidence-backed, при этом `baseline-subagents.yaml` может оставаться baseline bundle.
