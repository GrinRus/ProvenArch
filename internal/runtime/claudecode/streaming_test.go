...
func TestForwardStreamOutputEnforcesHardCapAndTruncates(t *testing.T) {
	...
	forwardStreamOutput(task, acpruntime.OutputStreamStdout, "abcdef\nanother-line\n", budget)
	...
	if chunks[1].Stream != acpruntime.OutputStreamStdout || !chunks[1].Truncated {
...
func TestCaptureCommandStreamForwardsCRLFAndFinalLineWithoutNewline(t *testing.T) {
	...
	if err := captureCommandStream(strings.NewReader(input), &sink, task, acpruntime.OutputStreamStdout, nil); err != nil {
	...
	if chunks[0].Stream != acpruntime.OutputStreamStdout || chunks[0].Text != "first line" || chunks[0].Truncated {
		t.Fatalf("unexpected first chunk: %+v", chunks[0])
	}
	if chunks[1].Stream != acpruntime.OutputStreamStdout || chunks[1].Text != "second line without trailing newline" || chunks[1].Truncated {
