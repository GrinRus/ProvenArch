    ...
    <section className="panel stage-panel task-inbox" data-testid="task-route-inbox">
      ...
      <div className="task-inbox-groups" data-testid="task-inbox-groups">
        {groups.filter((group) => grouped[group.id].length > 0).map((group) => <section className="task-group" key={group.id} data-testid={`task-group-${group.id}`} aria-labelledby={`task-group-${group.id}-title`}>
          ...
          <div className="task-row-list">{grouped[group.id].map((task) => <TaskRow key={task.task_id} task={task} group={group.id} selected={task.task_id === selectedTask?.task_id} onSelect={() => { setSelectedTaskId(task.task_id); onSelectTask?.(task.task_id, filters); }} />)}</div>
        </section>)}
        {groups.some((group) => grouped[group.id].length === 0) ? <details className="task-empty-groups"><summary>Empty lifecycle groups <span className="status info">{groups.filter((group) => grouped[group.id].length === 0).length}</span></summary><div>{groups.filter((group) => grouped[group.id].length === 0).map((group) => <section className="task-group" key={group.id} data-testid={`task-group-${group.id}`} aria-labelledby={`task-group-${group.id}-title`}><header className="task-group-heading"><h2 id={`task-group-${group.id}-title`}>{group.label}</h2><span className="status info">0</span></header><p className="hint task-group-empty">No Tasks</p></section>)}</div></details> : null}
      </div>
...
function TaskFiltersBar({ filters, onChange }: { filters: TaskFilters; onChange: (filters: TaskFilters) => void }) {
  ...
  const advancedCount = [filters.runner, filters.repository, filters.from, filters.to].filter((value) => Boolean(value)).length;
  return <form className="task-filters" aria-label="Task filters" onSubmit={(event) => event.preventDefault()}>
    ...
    <details className="task-filters-advanced"><summary>Advanced filters <span className="status info">{advancedCount > 0 ? `${advancedCount} active` : "Optional"}</span></summary><div className="task-filters-advanced-grid">
...
function TaskOutcome({ task, latestAttempt, review, onOpenArchitecture, onOpenChanges }: { task: ProductTask; latestAttempt?: TaskAttempt; review: RunReviewSummaryResponse | null; onOpenArchitecture?: (taskId: string) => void; onOpenChanges?: (taskId: string, attemptId: string, runId: string) => void }) {
  ...
  return <section className={`task-outcome task-outcome-${result.state}`} data-testid="task-outcome"><div className="task-outcome-copy"><p className="eyebrow">Outcome · exact run snapshot <code>{review.run_id}</code></p><h2>{result.state === "completed" ? "Architecture result ready" : result.state === "completed_with_gaps" ? "Result ready with gaps" : result.state === "canceled" ? "Attempt canceled" : "Attempt needs recovery"}</h2><p>{result.summary}</p><p className="hint">{result.promotion.current_usable ? "Current validator-approved Architecture remains available independently of this Task review." : "No current validator-approved Architecture is available."}</p></div><dl className="task-outcome-counts"><div><dt>Entities</dt><dd>{semantic ? `${semantic.entities_added} added · ${semantic.entities_changed} changed · ${semantic.entities_removed} removed` : producedMetric("entities")}</dd></div><div><dt>Edges</dt><dd>{semantic ? `${semantic.edges_added} added · ${semantic.edges_changed} changed · ${semantic.edges_removed} removed` : producedMetric("edges")}</dd></div><div><dt>Findings / questions</dt><dd>{semantic ? `${semantic.findings} / ${semantic.questions}` : "Unavailable"}</dd></div><div><dt>Gaps</dt><dd>{semantic ? semantic.gaps : result.coverage?.missing !== undefined ? `${result.coverage.missing} reported` : "Unavailable"}</dd></div></dl><div className="task-outcome-action"><strong>Recommended next action</strong><span>{result.recommended_action.replace(/_/g, " ")}</span>{onOpenArchitecture && result.promotion.current_usable ? <Button density="compact" onClick={() => onOpenArchitecture(task.task_id)} data-testid="task-open-architecture">Open current Architecture</Button> : null}{onOpenChanges && latestAttempt ? <Button density="compact" onClick={() => onOpenChanges(task.task_id, latestAttempt.attempt_id, review.run_id)} data-testid="task-open-changes">Open Changes and Publish</Button> : null}</div>{partial ? <p className="status warn">Semantic comparison is unavailable for this snapshot; no zero delta was fabricated.</p> : null}</section>;
}
...
function runnerLabelFromAttempt(attempt: TaskAttempt): string { return attempt.effective_runtime?.provider || attempt.intent_snapshot?.desired_runner?.provider || attempt.intent_snapshot?.desired_runner?.preset || "runner snapshot unavailable"; }
