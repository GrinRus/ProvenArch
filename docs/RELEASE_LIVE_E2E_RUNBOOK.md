...
- generic `codex` plugin/Cloudflare/state-db noise (`chatgpt.com/backend-api/plugins/featured`, Cloudflare HTML, `failed to renew cache TTL`, `state db`, `Operation not permitted`, MCP stream/list 504) учитывать как secondary telemetry; сами по себе такие строки не должны поднимать `runner_unavailable`, если raw runtime/session-summary не зафиксировали terminal provider failure. Default `codex-code` headless invocation запускается с `--ignore-user-config`, `--ignore-rules` и isolated auth-only `CODEX_HOME`; runtime копирует только caller `auth.json` и optional `installation_id`, но не пользовательские `config.toml`, MCP/plugins, skills, `.tmp/plugins` или rules, чтобы эти поверхности не задерживали artifact-only шаг до первого model action.
