...
- если run завершился `run_partial_failed` или `partial_failure_count>0` после failed/missing collect shard, primary class в batch/reporting остаётся collect-level `runtime_flow_failed`; `runner_unavailable`/`runtime_contract_failed` из classifier row или failed shard diagnostics фиксируются как secondary provider evidence и не должны переписывать root cause downstream `step2/3/4`;
