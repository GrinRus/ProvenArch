...
- `draft_artifact_enrichment` должен сохранять существующие `outputs[].path` и `outputs[].canonical_path` из текущего draft manifest; если manifest metadata обновляется, допустимы только top-level `summary`/`updated_at`, без пересборки `outputs[]` из provider-invented aliases.
