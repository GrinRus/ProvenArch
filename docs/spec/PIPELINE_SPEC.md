...
- legacy/final-index-like envelopes запрещены: top-level `pipeline`, `step`, `generated_at`, `domain_id`, `proposals[]`, `info_findings_noted`, `orphan_coverage_gaps` должны hard-fail-иться strict parser-ом; `updated_at` является единственным timestamp metadata key, разрешённым в runtime draft manifest;
