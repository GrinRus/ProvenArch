...
| Code quality audit remediation | in progress (Epic 19 / PR-1) | `docs/CODE_AUDIT_2026-07-10.md` + `docs/BACKLOG.md` Epic 19: crash consistency, lifecycle, contracts, UI correctness/accessibility and deterministic CI; PR-1 has committed `19A` atomic persistence/run-history last-good recovery, `19B` transactional canonical promotion, `19C` async panic isolation and `19D` server-owned shutdown, and is implementing `19E` coherent API session generation so workspace/runtime replacements conflict with active/pending work instead of mixing service ownership. Local frontend security hardening remains Wave 1+ non-goal |
