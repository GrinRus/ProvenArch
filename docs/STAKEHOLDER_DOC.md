...
| Task-first UI, runner presets and content-aware artifact workbenches | W23B1–W23N additive route/composer/Inbox/outcome/Pipeline Studio/Architecture-context/Markdown/model/Evidence/review/Changes/Ask/settings/state-quality foundation implemented; shell cutover pending (Epic 23) | [`spec/TASK_SPEC.md`](spec/TASK_SPEC.md), [`UI_TASK_FIRST_PRODUCT_DESIGN.md`](UI_TASK_FIRST_PRODUCT_DESIGN.md), [`UI_TASK_FIRST_PRODUCT_MIGRATION_PLAN.md`](UI_TASK_FIRST_PRODUCT_MIGRATION_PLAN.md) and 2026-08-11 Task/Attempt ADRs fix the target identity/persistence/admission/publication boundary; typed Task/Attempt routes, a scope/runner-aware New Task composer, public Inbox/detail history, exact-run semantic outcome, Attempt-bound Pipeline Studio, explicit Task-scoped current Architecture, allowlisted Markdown reader/editor, read-only schema-aware entity/edge inspector, non-authoritative Mermaid Evidence Studio, filtered findings/question/gap queue, exact Task-scoped Changes context, explicit Ask/Runner authority boundaries and Task state/accessibility coverage now exist without legacy-run fallback or Tasks-primary cutover. |
