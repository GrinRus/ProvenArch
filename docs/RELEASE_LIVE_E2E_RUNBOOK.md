...
- draft enrichment markdown не должен публиковать raw structured evidence dumps (`{'id': ...}`, `documents=[{...}]`, `citations=[{...}]`, truncated JSON/Python repr). Provider обязан summarise counts, selected titles, citation ids и repo/path refs in readable markdown. `step2.asis_docs` не должен писать stale claims that current-run `final-run-index.json` / `citation-index.json` are missing or unavailable: these indexes are downstream final staging artifacts, so absent indexes during step2 should be omitted from the as-is report rather than promoted as operator evidence. `step4.proposals` также не должен писать stale claims вроде `No current-run final-run-index document list was available`; if index status is absent, omit it, and if the index is present, summarize the canonical document count from top-level `canonical_documents[]` in `staging/final/final-run-index.json` and citation count from top-level `citations[]` in `staging/final/citation-index.json`. Provider must not infer zero documents from missing/nonexistent `documents[]`, `checked_paths[]`, or validator checked paths. Parenthesized variants like `final-run-index.json (0 observed document entries)` are the same stale zero-document claim unless the provider has validated a true current-run zero-document index. Shared draft validation treats both classes as `runtime_contract_failed`, not as manual artifact-quality-only warnings.
