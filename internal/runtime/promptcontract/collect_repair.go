...
func composeQwenToolFirstCollectPromptWithTargets(task acpruntime.Task, validationErr error, docRel string, docTarget string, manifestTarget string, evidencePaths []string) string {
	lines := []string{
		...
		"- Markdown must be at most 3000 characters of concise operator-facing architecture evidence: observed components/config/runtime behavior, concrete file references, and honest gaps. Do not mention repair, providers, process mechanics, or use validator-rejected phrases such as bounded read, bounded evidence read, bounded collection, bounded pass, guessed path/file/evidence, or missing expected evidence.",
		...
		"- For this compact contract every semantic provenance.kind must be exactly observation. Do not use inference, assertion, observed, inferred, asserted, document, report, source, or any other value; omit unsupported claims and record an observed coverage gap instead.",
