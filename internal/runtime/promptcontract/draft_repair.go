...
func ComposeDraftArtifactRepairPrompt(provider acpruntime.Provider, task acpruntime.Task, validationErr error) string {
	...
	lines := []string{
		...
		"- This structural recovery invocation has exactly one provider action: execute the exact shell command below and then stop. Do not call read_file, glob, or another read-only tool first, and do not replace the command with a generated Python program.",
	...
	case "init.step2.asis_docs", "refresh.step2.asis_docs":
		lines = append(lines,
			...
			"- This repair invocation is structural only: do not replace the bootstrap scaffold with evidence-backed prose or generate a second script. A separate enrichment invocation will rewrite all markdown from evidence.",
	...
	case "init.step4.proposals", "refresh.step4.proposals":
		lines = append(lines,
			...
			"- This repair invocation is structural only: do not replace the bootstrap scaffold with evidence-backed prose or generate a second script. A separate enrichment invocation will rewrite proposal/changelog content from evidence.",
...
func ComposeDraftArtifactEnrichmentPrompt(provider acpruntime.Provider, task acpruntime.Task, validationErr error) string {
	...
	lines := []string{
		...
		"- The first provider action must be one shell/terminal command that both reads bounded evidence and writes every referenced markdown target. Do not call read_file, glob, or another read-only tool before that command; a sequence of tool reads without a write is a stalled recovery.",
		"- Validation errors are diagnostic input only: never copy their absolute paths, reports/taskruns or staging paths, empty shell-expanded slots, or recovery wording into user-visible markdown.",
		...
		"- Keep evidence references literal and deterministic: resolve finding IDs and repo:path values into shell variables before writing, and never allow an unset variable to produce text such as 'from  and', 'checked:  and', or an empty Finding ID.",
		...
		"- If you use Python, build markdown from complete single-line string values and join them with an escaped newline (\\n) or chr(10). Never place a physical newline inside a quoted Python string; a SyntaxError leaves the recovery with no artifacts. Do not print a generated script as prose: execute it in the same command.",
	...
	case "init.step2.asis_docs", "refresh.step2.asis_docs":
		lines = append(lines,
			...
			"- Repo evidence references in overview.md must use exact non-root paths. Never write repository-root shorthand such as repo:. or repo:./, and never write glob or wildcard syntax such as repo:src/*; cite a concrete existing file or directory instead.",
			...
			"- In overview.md, never use current run/current-run, typed shard, shard pack/manifest, or planned/succeeded/failed/incomplete counter wording; those execution details belong only in summary.md and architect-summary.md. Also never mention run IDs, pipeline step names, collect passes, repair/enrichment attempts, staged-source mechanics, or how runtime assembled the document; state repository facts and evidence gaps directly.",
...
func composeDraftArtifactEnrichmentCompactStep2RetryPrompt(provider acpruntime.Provider, task acpruntime.Task, manifestFile string, manifestTarget string, outputs []runtimedrafts.Output, statusEvidenceFiles []string, validationErr error) string {
	lines := []string{
		...
		"- Use the shell/terminal command tool for that action. Do not call read_file, glob, or another read-only tool first; separate tool reads are not an acceptable substitute for the bounded write command.",
		...
		"- Shell assignments such as MANIFEST=/path are not exported to child processes. If Python needs a path, pass it as a positional argument (for example: python3 - \"$MANIFEST\" <<'PY' and read sys.argv[1]) or export the variable explicitly before invoking Python; never use os.environ[...] with a bare, unexported NAME=/path assignment.",
		"- If you use Python, build markdown from complete single-line string values and join them with an escaped newline (\\n) or chr(10). Never place a physical newline inside a quoted Python string; syntax errors are terminal for this bounded retry.",
...
func composeDraftArtifactEnrichmentCommandTextRetryPrompt(provider acpruntime.Provider, task acpruntime.Task, manifestFile string, manifestTarget string, outputs []runtimedrafts.Output, statusEvidenceFiles []string, validationErr error) string {
	lines := []string{
		...
		"- Use the shell/terminal command tool for that action. Do not call read_file, glob, or another read-only tool first; the command must execute and mutate files before any assistant text.",
