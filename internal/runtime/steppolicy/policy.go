...
func DocFirstFilesystemPolicy(task acpruntime.Task) string {
	...
	case "init.step2.asis_docs", "refresh.step2.asis_docs":
		lines = append(lines,
			...
			`- Architecture Home must never reference reports/taskruns/**, taskrun staging paths, write_root, draft_final_root, or raw runtime artifacts; navigation must use canonical reports/model/proposals paths or concrete repository paths.`,
			`- Architecture Home must not mention run IDs, pipeline step names, collect passes, repair/enrichment attempts, or how runtime assembled the document; state repository facts and evidence gaps directly.`,
