...
| Code quality audit remediation | in progress (Epic 19 / PR-1) | `docs/CODE_AUDIT_2026-07-10.md` + `docs/BACKLOG.md` Epic 19: crash consistency, lifecycle, contracts, UI correctness/accessibility and deterministic CI; PR-1 has committed `19A` atomic persistence/run-history last-good recovery, `19B` transactional canonical promotion and `19C` async panic isolation, and is implementing `19D` server-owned shutdown for bounded serve cancellation and active/pending run cleanup. Local frontend security hardening remains Wave 1+ non-goal |
