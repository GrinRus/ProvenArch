...
func TestDocFirstFilesystemPolicyDefinesSharedCollectRepairSurface(t *testing.T) {
	...
	required := []string{
		...
		`Collect authored markdown must never publish internal execution paths such as .acp/repos/, reports/taskruns/, staging/final/, staging/shards/, write_root, or draft_final_root`,
...
func TestAsIsFirstActionSectionWritesEvidenceBackedDraftSetFirst(t *testing.T) {
	...
	required := []string{
		...
		`immediately following filesystem action must write overview.md`,
		...
		"bounded read/list command must not use `for f` loops",
		`If that one read command fails, do not retry it`,
		"direct-literal write command must not use loops, shell variables, command substitution",
...
func TestDocFirstFilesystemPolicyDefinesCanonicalAsIsDraftSurface(t *testing.T) {
	...
	required := []string{
		...
		`run at most one bounded staged-evidence read/list command`,
