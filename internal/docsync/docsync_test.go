...
func TestMultiRepoE2EDocsAreConsistent(t *testing.T) {
	...
	required := map[string][]string{
		".agents/skills/e2e-live-gate/SKILL.md": {
			...
			"machine execution verdicts only",
			"accepted SWE UX and artifact-quality reports",
			"Harness больше не пишет `blackbox_e2e_steps_*`",
			"goal:",
			"action:",
			"observed evidence:",
		...
		"docs/RELEASE_LIVE_E2E_RUNBOOK.md": {
			...
			"not GitHub Actions workflow",
			"Scripts produce setup/execution artifacts and machine execution verdicts only",
			"operator_blackbox_assessment_<matrix-id>.md",
			...
			"backend execution evidence inspection",
			"frontend UI/API init inspection and UX evidence capture",
			"release execution verdict verification",
			"SWE UX assessment and SWE artifact-quality assessment",
		...
		"docs/TESTING_STRATEGY.md": {
			...
			"Scripts produce machine execution evidence and verifier-backed execution verdicts only",
			"harness no longer generates `blackbox_e2e_steps_*`",
			...
			"matrix_result_<matrix-id>.json/.md",
		},
	}
