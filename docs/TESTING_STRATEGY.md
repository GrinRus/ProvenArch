...
- strict collect validation:
  ...
  - collect pair recovery запускается один раз при no authored artifacts + non-empty provider diagnostics или exhausted fully silent collect fresh retry, разрешает писать только suggested authored doc + `shard-pack-manifest.json`; first recovery write является marker-free seed fallback и должен быть rewritten evidence-backed перед success, а `Recovery Summary`/`Recovery Bootstrap`/`Recovery Evidence Summary` scaffold reject-ится
