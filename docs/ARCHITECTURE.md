...
9) **Runtime Profile API (`internal/api`)** *(implemented baseline)*
   ...
   - live harness DoD/UI precheck runs before product runtime execution; exact Node/npm, `make contracts test lint build`, `npm ci --prefix ui` and `playwright install chromium` failures materialize as execution `precheck_failed`. UI dependency/browser commands are bounded by `ACP_LIVE_PRECHECK_UI_TIMEOUT_SEC`; timeout evidence lives in precheck logs and must not be classified as provider runtime, frontend UX, or artifact-quality verdict.
