    ...
    <section className="settings-page" data-testid="settings-page">
      ...
      <section className="settings-shell" aria-label="Workspace settings">
        ...
        <div className="settings-main">
          ...
          <section className="settings-card settings-runtime-card" id="settings-runtime" data-testid="settings-runtime"><div className="settings-card-heading"><div><p className="eyebrow">Analysis runtime</p><h3>{runtimeLabel || "Runtime profile"}</h3><p className="hint">Timeouts, execution policy, permissions and provider models shared with Setup.</p></div><span className="status info">Persisted profile</span></div>{runtimeSettingsPanel}</section>
          <div className="settings-secondary-grid"><section className="settings-card" id="settings-appearance" data-testid="settings-appearance"><p className="eyebrow">Appearance</p><h3>Product defaults</h3><p>Keyboard focus and reduced-motion preferences follow local console defaults.</p></section><section className="settings-card" id="settings-diagnostics" data-testid="settings-diagnostics"><p className="eyebrow">Diagnostics</p><h3>Readiness and recovery</h3><p>Provider checks, permission recovery and run logs remain available from Analyze.</p><button type="button" className="link-button" onClick={onOpenRuns}>Open Analyze diagnostics</button></section></div>
        </div>
      </section>
