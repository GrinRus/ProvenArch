...
- `codex-code` collect steps use a 3-minute initial pre-artifact stall window. This is provider policy, not matrix timeout override: if no authored collect artifacts appear within that window, the step still follows the normal stall/focused-repair classification path. Write-first collect and focused repair commands also have an absolute pre-artifact wall-clock cap, so stdout/stderr-only stream activity cannot indefinitely extend a no-artifact command.
...
- `init.step0.constitution` draft enrichment отличается от `step2/step4`: collected shard manifests, final indexes и validator output на этом шаге ещё не существуют. Если bootstrap-only `charter-overview.md` дошел до enrichment, focused prompt обязан как следующий filesystem action прочитать текущий draft manifest/current charter overview плюс bounded repo entrypoint evidence from selected repo roots, затем fresh-overwrite exact `draft_final_root/charter-overview.md` before optional analysis. Валидный enrichment содержит target identity, repo/path evidence references, architecture scope, operating constraints, coverage gaps and decision-ready operator summary; sparse repo evidence записывается как explicit gap. `baseline-subagents.yaml` сохраняется, если он уже является валидным baseline bundle. Step0 enrichment uses a bounded 2-minute pre-artifact window, and its compact no-action retry names the exact `charter-overview.md` target plus bounded repo entrypoint candidates. Для `git_url` repo resolved checkout может быть доступен только как `read_context_roots` path under `.acp/repos`; step0 enrichment includes that selected repo root directly while excluding workspace root and taskrun/report roots. Noop, stall или scaffold-preserving output остаются `runtime_contract_failed` / `draft_artifact_enrichment_noop_or_scaffold`; ACP не синтезирует constitution markdown вместо provider.
...
- Если provider-authored `draft_artifact_enrichment` завершился/stalled without a fresh valid rewrite and validation still reports `draft_artifact_enrichment_noop_or_scaffold`, shared runtime может выполнить ровно один compact provider-authored retry `draft_artifact_enrichment_no_action_retry`. Retry prompt is shorter and command-only: next provider action must be one `python3` filesystem command that reads bounded current-run manifest/status/index/finding/shard evidence and overwrites every referenced markdown target before returning. Это не deterministic synthesis and not a quality bypass; repeated no-op/scaffold/malformed/invalid output remains terminal `runtime_contract_failed`, and `repair_exhausted` is emitted only after the retry is exhausted.
- Если `draft_artifact_enrichment_no_action_retry` возвращает shell/Python command text (например `python3 - <<...` или fenced bash/python) без fresh filesystem mutation, shared runtime может выполнить ровно один provider-authored `draft_artifact_enrichment_command_text_retry`. Retry существует только для принудительного real command execution после printed-command output; success still requires marker-free fresh rewrites, and repeated printed-command/noop/scaffold output remains terminal `runtime_contract_failed`.
...
- `step4.proposals` enrichment must count indexed documents from top-level `final-run-index.json.canonical_documents[]` and citations from top-level `citation-index.json.citations[]`; providers must not infer `0 observed document entries` from missing `documents[]`, `checked_paths[]`, or validator checked paths.
