...
func StepSpecificPolicy(stepID string) string {
	...
	case "init.step2.asis_docs", "refresh.step2.asis_docs":
		return strings.Join([]string{
			...
			`- Evidence-first draft requirement: the first bounded filesystem action sequence must read current-run staged evidence, write overview.md, summary.md, and architect-summary.md, then write asis-draft-manifest.json last before any final answer, status note, or optional extra analysis.`,
			...
			`- Markdown writes must be shell-safe direct literal writes using single-quoted heredocs (<<'EOF'); before all four required artifacts exist, do not use Python, Node, Ruby, Perl, awk, jq, generated source-code strings, template programs, or nested quote tricks to assemble markdown/JSON.`,
...
func DocFirstFilesystemPolicy(task acpruntime.Task) string {
	...
	case "init.step2.asis_docs", "refresh.step2.asis_docs":
		lines = append(lines,
			...
			`- Every Architecture Home section must cite a concrete repository path or canonical document when evidence exists. Do not write manifest recaps, provider/runtime narration, taskrun mechanics, or confidence claims unsupported by evidence.`,
			`- Every repo:path reference in Architecture Home must name an existing file or directory under that repository's current read root. If a candidate path cannot be resolved, state it as an evidence gap without presenting the missing path as evidence.`,
			...
			`- Before writing asis-draft-manifest.json, scan every repo:path token in Architecture Home and reject any path containing *, ?, [, ], {, or }; never compress multiple concrete paths into a wildcard reference.`,
...
func AsIsFirstActionSection(task acpruntime.Task) string {
	...
	lines := []string{
		...
		"- This as-is step must start with one bounded evidence-read/write action sequence, not with a bootstrap-only draft.",
		"- The first filesystem action may perform one bounded current-run evidence read/list. The immediately following filesystem action must write overview.md, summary.md, and architect-summary.md under draft_final_root, then write asis-draft-manifest.json last before returning.",
		...
		"- overview.md is the canonical Architecture Home. It must contain non-empty sections named exactly: System at a glance; Analyzed scope; Domains and ownership; Key flows; Integrations and datastores; Where to start; Safe-change guidance; Evidence gaps and open questions.",
		"- Write every required Architecture Home heading as its own Markdown H2 line (for example, \"## System at a glance\"), followed by the substantive section body on later lines; never append body text to the heading line.",
		"- Architecture Home sections must use concrete repo/path, citation, or canonical-document refs when evidence exists. Manifest recaps, runtime/provider narration, and unsupported confidence claims are invalid.",
		"- Every repo:path reference in Architecture Home must name an existing file or directory under that repository's current read root. Unresolved candidates must be described as evidence gaps, not published as evidence references.",
		"- Repo evidence references must use exact non-root paths. Never publish repository-root shorthand such as repo:. or repo:./, and never use glob or wildcard syntax such as repo:src/*; cite a concrete existing file or directory instead.",
		"- Before writing asis-draft-manifest.json, scan every repo:path token in Architecture Home and reject any path containing *, ?, [, ], {, or }; never compress multiple concrete paths into a wildcard reference.",
		"- Architecture Home must never reference reports/taskruns/**, taskrun staging paths, write_root, draft_final_root, raw runtime artifacts, absolute runtime checkout paths, or .acp/repos paths; navigation must use canonical reports/model/proposals paths or stable repo-relative references such as bank-of-anthos:src/frontend.",
		"- Architecture Home must not use current run/current-run, typed shard, shard pack/manifest, or planned/succeeded/failed/incomplete counter wording; those execution details belong only in summary.md and architect-summary.md. It also must not mention run IDs, pipeline step names, collect passes, repair/enrichment attempts, staged-source mechanics, or how runtime assembled the document; state repository facts and evidence gaps directly.",
		...
		"SHELL-SAFE MARKDOWN: write repository paths and canonical paths as plain text in the first-pass markdown. Do not use Markdown backticks or fenced code blocks in heredoc content, and do not use backtick command substitution or $() in the write command; the provider shell may wrap the command and must not erase evidence references.",
	...
	lines = append(lines,
		...
		"- overview.md contains all eight required Architecture Home sections with concrete evidence references or explicit evidence gaps.",
		"- Every repo:path token in overview.md names one concrete path and contains none of *, ?, [, ], {, or }.",
		...
		"- Before the manifest write, the same command checked overview.md, summary.md, and architect-summary.md case-insensitively and none contains bounded evidence read, bounded staged evidence, bounded read root/pass, recovery pass/action, enrichment read/pass, draft surface/root/manifest, bootstrap, placeholder replacement, runtime provider, or generated/assembled-by-runtime narration.",
...
func asIsFirstPassWriteSequence(task acpruntime.Task) string {
	...
	return strings.Join([]string{
		...
		"- For this first-pass as-is write, prefer plain repository:path and canonical path text without Markdown backticks or fenced code blocks; this keeps nested provider shell wrappers from evaluating evidence references.",
...
func ProposalsFirstActionSection(task acpruntime.Task) string {
	...
	lines := []string{
		...
		"- Treat every reports/taskruns/** or staging/** path as an input locator only. Do not copy those paths into proposal.md or changelog.md; use canonical reports/findings/findings.md, reports/coverage/summary.md, proposal/report paths, exact finding/citation IDs, and stable repo:path evidence in final markdown.",
	...
	lines = append(lines,
		...
		"- No markdown target contains reports/taskruns/**, staging/final/**, staging/shards/**, write_root, or draft_final_root paths.",
		"- No markdown target says it is a bootstrap, placeholder, draft surface initialized, recovery output, or content that will be replaced later.",
	)
	return strings.Join(lines, "\n")
}
