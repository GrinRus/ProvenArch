    ...
    <main className="product-shell" data-testid="product-shell">
      ...
      <div className={`product-layout ${navCollapsed ? "nav-collapsed" : ""}`}>
        <nav className="primary-nav" aria-label="Primary">
          ...
          {destinations.map((item) => <a key={item.id} href={destinationPaths[item.id]} data-testid={destinationTestIDs[item.id] ?? `destination-${item.id}`} aria-current={destination === item.id ? "page" : undefined} onClick={(event) => { event.preventDefault(); onDestinationChange(item.id); }}><span aria-hidden="true">{item.label.slice(0, 1)}</span><span className="nav-label">{item.label}</span></a>)}
        </nav>
