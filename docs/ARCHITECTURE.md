...
5) **Runtime providers (`internal/runtime/*`)** *(implemented baseline)*
   ...
   - collect prompt ставит command-first `FIRST COLLECT ARTIFACT PAIR COMMAND` сразу после provider identity: suggested authored doc path и literal task-specific `shard-pack-manifest.json` skeleton должны быть записаны как focused artifact pair до broad second-pass sweep. До этого command нельзя делать `read_file`/`list_directory`/`grep_search`/`glob` или любую repo exploration; repo entrypoint hints в collect prompt явно отложены until after the first pair exists. Поздний docs-first block только ссылается на этот skeleton и не дублирует heredoc command; после heredoc provider обязан сделать targeted evidence pass, перезаписать authored doc + manifest, убрать `ACP_COLLECT_BOOTSTRAP_REPLACE_BEFORE_EXIT` и не завершаться с generic owner-gap/contains-only semantic. Collect artifact monitor дает bootstrap/partial collect artifacts extended enrichment window перед post-artifact stall/recovery, чтобы live provider успевал заменить marker scaffold реальным evidence-backed content. Неизменённая bootstrap collect pair попадает в `artifact_quality:*` warnings и блокирует live quality gate. `refresh.step1.collect` skeleton сразу содержит policy-minimum `coverage.missing` и stable `questions[]`, чтобы first-action heredoc не конфликтовал с refresh policy; для `init.step1.collect` / `refresh.step1.collect` runtime выполняет максимум одну collect pair-recovery попытку, если provider оставил stdout/stderr diagnostics, не записал authored artifacts или записал только bootstrap-only authored doc, и максимум одну manifest-only repair попытку, если provider уже записал non-bootstrap authored docs в `write_root`, но `shard-pack-manifest.json` отсутствует или невалиден; repair prompts начинаются с `FIRST COLLECT MANIFEST REPAIR COMMAND` / command-first absolute heredoc write targets вокруг task-specific skeletons, используют тот же root-file primary evidence preference, instruct overwrite вместо read/diff/patch invalid manifests, не повторяют validation-error field cues, а engine write-set guards разрешают только expected pair или `write_root/shard-pack-manifest.json`
   ...
   - collect step не считается успешным, если после разрешённых focused collect recovery попыток `shard-pack-manifest.json` остаётся missing/invalid, `documents[].path` ссылается на отсутствующий authored doc или referenced authored doc сохраняет bootstrap marker/scaffold prose; такой случай поднимается как runtime contract failure (`runtime_contract_failed`) и hard pass невозможен. Collect pair-recovery command пишет marker-free validation-ready recovery fallback pair с concrete scoped path evidence и recovery semantic finding; если provider продолжает выполнение, он может выполнить targeted evidence pass и заменить fallback более богатым content, но первый recovery write уже не должен быть bootstrap-only. Marker-free `Recovery Summary`/`Recovery Bootstrap` scaffold также reject-ится, чтобы repair не закреплял low-signal recovery-only docs. Fully silent no-artifact qwen/claude collect path после bounded retry остаётся `runner_unavailable`
