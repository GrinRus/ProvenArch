...
- collect prompt ставит suggested authored doc path и literal task-specific `shard-pack-manifest.json` skeleton в command-first section сразу после provider identity, чтобы provider писал bootstrap manifest сразу после первого authored shard doc, а не после broad repository sweep; до этого command provider не должен делать `read_file`, `list_directory`, `grep_search`, `glob` или repo exploration, а repo entrypoint hints в collect prompt применимы только после того, как first artifact pair уже существует. Эта heredoc command встречается один раз, а поздний docs-first block только ссылается на first-action skeleton; first-action skeleton не является final-acceptable content surface: после command provider обязан сделать один targeted evidence pass по entrypoint hints/path scopes, перезаписать authored doc + `shard-pack-manifest.json`, удалить `ACP_COLLECT_BOOTSTRAP_REPLACE_BEFORE_EXIT` и заменить generic owner-gap/contains-only semantic на evidence-backed content там, где repo evidence это поддерживает. Collect artifact monitor применяет extended enrichment window для collect post-/partial-artifact stalls, чтобы marker-bearing bootstrap pair не уходила в repair раньше targeted evidence pass. Неизменённая bootstrap pair помечается как `artifact_quality:*` и блокирует live quality gate.
