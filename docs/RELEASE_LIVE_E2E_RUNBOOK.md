...
- backend-cycle helper под `scripts/full-run-batch.sh` должен писать failed headless `init`/`refresh` rows в существующем 17-field `run-results.tsv` формате, когда CLI уже вернул `run_id`; если CLI не напечатал `run_id`, helper должен best-effort восстановить newest `run_*` из workspace quality/raw metadata и всё равно append-ить failed row. Best-effort snapshot выполняется даже при missing/invalid run telemetry summary; runtime artifacts/logs после terminal provider/runtime failure не должны выглядеть как missing-row infra gap;
