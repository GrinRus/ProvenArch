...
- Normal step2 first-pass tests require the same initial filesystem command to write all three Markdown targets, scan `overview.md`, `summary.md`, and `architect-summary.md` case-insensitively for the enforced runtime/recovery marker classes, rewrite any match as an architecture fact, coverage gap or operator decision, and only then write `asis-draft-manifest.json` last. A reduced provider-authored fixture protects the live-observed `bounded evidence read` / runtime-assembly narration regression without adding matrix identity or live-only behavior to production code.
...
- strict collect validation:
  ...
  - `fixtures/scenarios/collect-manifest-wrong-artifact-root` проверяет identity drift без live-specific данных: structurally valid manifest с существующим authored document, но foreign `artifact_root`, fail-ится до downstream use. Providercommon engine regression подтверждает, что ordinary provider-authored manifest repair может записать точную task identity и пройти тот же strict gate; ACP и live harness не нормализуют path вручную.
