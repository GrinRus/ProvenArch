...
| Task-first UI, runner presets and content-aware artifact workbenches | W23B1–W23F additive route/composer/Inbox/outcome/Pipeline Studio foundation implemented; UI wave and cutover pending (Epic 23) | [`spec/TASK_SPEC.md`](spec/TASK_SPEC.md), [`UI_TASK_FIRST_PRODUCT_DESIGN.md`](UI_TASK_FIRST_PRODUCT_DESIGN.md), [`UI_TASK_FIRST_PRODUCT_MIGRATION_PLAN.md`](UI_TASK_FIRST_PRODUCT_MIGRATION_PLAN.md) and 2026-08-11 Task/Attempt ADRs fix the target identity/persistence/admission/publication boundary; typed Task/Attempt routes, a scope/runner-aware New Task composer, public Inbox/detail history, exact-run semantic outcome and Attempt-bound Pipeline Studio now exist without legacy-run fallback or Tasks-primary cutover. |
