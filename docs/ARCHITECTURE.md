...
3) **Orchestrator (`internal/orchestrator`)** *(implemented baseline)*
   ...
   - generic `codex` plugin/Cloudflare/state-db warnings (`plugins/featured`, Cloudflare HTML, cache/state-db permission noise) считаются secondary telemetry и не должны сами поднимать `runner_unavailable`; default `codex-code` headless invocation uses `--ignore-user-config` + `--ignore-rules` and an auth-only isolated `CODEX_HOME` copied from the caller's `auth.json`/`installation_id`, so user config, MCP/plugins, skills and rules are not part of ACP artifact-only tasks before model actions
