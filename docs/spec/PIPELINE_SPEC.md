...
- `draft_artifact_enrichment` для `step2` обязан читать текущий draft manifest, staged shard manifests/authored docs/final indexes/citation indexes и переписать `overview.md`, `summary.md`, `architect-summary.md` с repo/path evidence, coverage gaps и decision-ready operator summary; unchanged files или scaffold markers остаются contract failure, а не artifact-quality-only warning.
