...
- collect prompt ставит suggested authored doc path и literal task-specific `shard-pack-manifest.json` skeleton в command-first section сразу после provider identity, чтобы provider писал marker-free validation-ready collect pair сразу после первого authored shard doc, а не после broad repository sweep; до этого command provider не должен делать `read_file`, `list_directory`, `grep_search`, `glob` или repo exploration, а repo entrypoint hints в collect prompt применимы только после того, как first artifact pair уже существует. Эта heredoc command встречается один раз, а поздний docs-first block только ссылается на first-action skeleton; first-action pair является minimal validation-ready scoped evidence surface: после command provider должен сделать один targeted evidence pass по entrypoint hints/path scopes и перезаписать authored doc + `shard-pack-manifest.json` более богатым evidence-backed content там, где repo evidence это поддерживает. Collect artifact monitor применяет extended enrichment window для collect post-/partial-artifact stalls перед repair; если provider stalls после marker-free valid pair, validation может завершить shard без recovery-only fallback. Marker-bearing/unchanged legacy bootstrap collect docs всё ещё помечаются как `artifact_quality:*` и блокируют live quality gate.
- collect runtime до финальной validation может выполнить focused provider-authored recovery без ACP-side synthesis: если provider оставил stdout/stderr diagnostics, но не записал authored artifacts, общий engine делает одну collect pair-recovery попытку с write-set guard только на suggested authored doc + `write_root/shard-pack-manifest.json`; если provider уже записал bootstrap-only authored doc с `ACP_COLLECT_BOOTSTRAP_REPLACE_BEFORE_EXIT` или unchanged scaffold prose, collect validation reject-ит такой referenced document и recovery выбирает pair-recovery, чтобы provider мог переписать markdown + manifest, а не закреплять плохой doc manifest-only repair-ом; если provider уже записал non-bootstrap authored docs, но `shard-pack-manifest.json` missing/invalid, общий engine делает одну manifest-only repair попытку с write-set guard только на `write_root/shard-pack-manifest.json`; ACP не нормализует `documents[].path`, не автозаполняет metadata и не использует semantic stdout repair
...
- collect pair recovery prompt содержит command-first absolute heredoc write targets для suggested authored doc + task-specific manifest skeleton; pair-recovery heredoc является marker-free validation-ready recovery fallback surface с concrete scoped path evidence, recovery semantic finding и explicit coverage gap. Если provider продолжает выполнение после first write, он может выполнить targeted evidence pass и заменить fallback более богатым content, но первый recovery write уже должен проходить strict collect validation. Marker-bearing docs, `Recovery Bootstrap`/`Recovery Summary` scaffold и marker-free low-signal recovery summaries reject-ятся тем же collect validation, чтобы short valid-artifact stop не закреплял recovery-only docs. Manifest-only repair prompt начинается с `FIRST COLLECT MANIFEST REPAIR COMMAND` и сразу даёт absolute heredoc write target с task-specific JSON skeleton, overwrite-only instruction для existing invalid manifests и compact validation checklist; authored docs/evidence уже encoded в skeleton, root-file repair evidence использует тот же `README` -> `Makefile` -> build/deploy preference, а validation-error details не повторяются как field-level patch cues. Это не ACP-side manifest synthesis, а provider instruction surface под write-set guard.
