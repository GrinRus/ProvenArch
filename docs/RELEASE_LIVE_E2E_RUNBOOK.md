...
- `qwen-code` для `init.step1.collect` / `refresh.step1.collect` дополнительно имеет internal stall watchdog в двух фазах:
  - `pre_artifact`: если provider жив, но до первого authored artifact одновременно нет stdout/stderr activity и нет мутаций `write_root`, runtime завершает process принудительно и делает forced fresh retry; на этом retry pre-artifact sentinel не выключается полностью, а получает расширенное bounded grace window, чтобы второй запуск не крутился до полного `step_timeout` на silent provider; если и это окно выгорает, runtime пишет `retry exhausted`, и fully silent retry exhaustion (`stdout/stderr == empty`) считается `runner_unavailable`, а retry с provider output остаётся `runtime_contract_failed`;
  - `post_artifact`: после появления `shard-pack-manifest.json` + authored docs runtime отслеживает реальную pipe activity и мутации `write_root`; если оба сигнала молчат один internal stall window, provider process завершается принудительно, runtime пишет diagnostic events (`runtime task stalled before artifacts` / `runtime task stalled after artifacts` / `retry scheduled` / `retry completed` / `retry exhausted`) и делает forced `RETRY RECOVERY MODE` до step timeout.
...
- если collect evidence = `unusable`, `init|refresh.step2.asis_docs` не должен запускать live provider: staged final set собирается только из persisted collect artifacts и дальше остаётся triage-only incomplete surface.
