...
- Compact `collect_pair_repair` manifests must include `citations[].document_ids` for every citation, pointing at existing `documents[].id`. If process-contaminated markdown was fresh-rewritten but the resulting manifest is still schema-invalid only around manifest binding/semantic shape, runtime may chain into manifest-only repair; this fallback must not run when the markdown stayed stale, process-contaminated, bootstrap-only, or still cites missing repo evidence.
