...
- collect prompt ставит suggested authored doc path и literal task-specific `shard-pack-manifest.json` skeleton в `COLLECT EVIDENCE-FIRST ARTIFACT PAIR` section сразу после provider identity; skeleton является schema/key/type guide, а не heredoc artifact. Первый collect filesystem action должен быть bounded evidence-write action: provider читает только existing repo entrypoint hints и assigned `path_scopes` (до 8 representative files, до 6000 bytes per file; oversized files truncate/skip with continue), не читает `reports/taskruns/**`, raw logs, sibling shard manifests или archive docs, и до возврата пишет marker-free evidence-backed authored doc + `shard-pack-manifest.json`. Analysis-only/status/progress prose, todo/planning, read-only preflight, broad repo sweep, generated source-code strings, Python f-strings, `.format(...)` template writers и nested quote tricks до этой записи запрещены; normal collect не должен рассчитывать на focused repair как штатный success path. Normal collect больше не пишет seed-only pair как первый filesystem action. Strict collect validation fail-ит marker-bearing docs, marker-free unchanged seed docs, recovery-fallback docs и scaffold-only semantic snapshot (`contains scoped surface` + generic owner mapping gap) вместо `artifact_only` success.
