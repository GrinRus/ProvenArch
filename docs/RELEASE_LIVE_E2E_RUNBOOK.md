...
- для `init.step1.collect` и `refresh.step1.collect` collect prompt начинает task-specific surface с `FIRST COLLECT ARTIFACT PAIR COMMAND` сразу после provider identity: suggested authored doc path + literal task-specific manifest skeleton должны быть записаны как focused marker-free seed pair до broad second-pass repository sweep. До этой команды provider не должен вызывать `read_file`, `list_directory`, `grep_search`, `glob` или repo exploration; repo entrypoint hints в collect prompt применяются только после того, как first artifact pair уже существует. Generic manifest examples не добавляются в collect prompts; task-specific skeleton является единственным JSON template, а поздний docs-first block не дублирует heredoc command. Normal first-action pair является seed-only write surface: normal prompt требует targeted evidence pass после heredoc и перезапись authored doc + manifest более богатым evidence-backed content там, где repo evidence это поддерживает. Collect artifact monitor использует extended enrichment window для collect partial/post-artifact stalls перед repair; если provider stalls с unchanged seed/recovery fallback pair, validation fail-ит shard вместо `artifact_only` success. Referenced authored docs с legacy marker/scaffold prose, marker-free unchanged seed prose или `Recovery Evidence Summary` fallback prose reject-ятся collect validation-ом. Общий runtime может сделать одну collect pair-recovery попытку только если provider оставил stdout/stderr diagnostics и не записал authored artifacts; bootstrap/seed-only authored docs не маскируются pair-recovery success. Manifest-only repair разрешён только для non-bootstrap authored docs с missing/invalid `write_root/shard-pack-manifest.json`. Pair recovery heredoc остаётся seed recovery surface и должен быть rewritten evidence-backed перед successful exit. Manifest-only repair пишет только `shard-pack-manifest.json` и начинается с `FIRST COLLECT MANIFEST REPAIR COMMAND` plus absolute heredoc write target вокруг exact skeleton. Repair prompts instruct overwrite for invalid manifests rather than read/diff/patch, не повторяют validation-error field cues, а engine write-set guards запрещают лишние writes.
