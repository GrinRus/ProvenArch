...
func ComposeCollectArtifactPairRepairPrompt(provider acpruntime.Provider, task acpruntime.Task, validationErr error) string {
	...
	lines := []string{
		...
		"- The command writes a marker-free, validation-ready recovery fallback pair with scoped evidence paths encoded in both the document and manifest.",
		...
		"- Exiting successfully immediately after the heredoc command is valid only when the written pair remains marker-free and shard-pack-manifest.json validates.",
