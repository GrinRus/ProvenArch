...
- Process-contaminated collect markdown is also contract-invalid: final collect docs must not narrate bounded reads/passes, guessed paths/files/evidence, expected-missing path checks, recovery attempts, or later repair. When existing authored markdown has this contamination, runtime routes to provider-authored collect pair repair with mandatory fresh rewrite of the same markdown target; manifest-only runtime recovery must not turn that document into hidden deterministic success.
