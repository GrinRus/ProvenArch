...
| Code quality audit remediation | in progress (Epic 19 / PR-1) | `docs/CODE_AUDIT_2026-07-10.md` + `docs/BACKLOG.md` Epic 19: crash consistency, lifecycle, contracts, UI correctness/accessibility and deterministic CI; PR-1 has committed `19A` atomic persistence/run-history last-good recovery and `19B` transactional canonical promotion, and is implementing `19C` async panic isolation so panicking async runners terminalize without keeping the service slot occupied. Local frontend security hardening remains Wave 1+ non-goal |
