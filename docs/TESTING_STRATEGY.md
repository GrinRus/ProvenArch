...
- runtime write audit regression tests require protected workspace, unclassified workspace, and analyzed repo mutations after provider execution to fail otherwise-successful steps as `runtime_contract_failed`; only explicit provider staging and orchestrator run log/history roots are allowed; incomplete workspace/repo snapshots fail closed as `runtime_write_audit_failed`; read-only isolated repo clones must pass through a lightweight HEAD/index/mode snapshot without writable `git status`; initial non-git read roots remain warning-only diagnostics, but an audited repo that becomes unreadable after runtime is fatal.
...
- async lifecycle operability:
  ...
  - server-owned shutdown tests cover active run context cancellation, queued pending run `run_canceled` terminalization without runner start, post-shutdown `ErrServiceClosed`, API `Serve` context cancellation waiting for orchestrator shutdown, and the async-run quiescence barrier that keeps final history writes inside the workspace lifetime
...
- docflow builder seam:
  ...
  - promotion builds a complete run-scoped generation, validates indexed files, activates managed canonical roots with journaled rollback, and regression tests inject failures across copy/model/diagram/activation operations plus concurrent symlink swaps to prove canonical state is either the previous complete generation or the new complete generation without outside-root mutation
...
- `scripts/full-run-batch.sh` — canonical live batch + frontend live e2e:
  ...
  - frontend smoke работает на отдельной `frontend-workspace` копии backend workspace, merge-ит выбранный run snapshot reports и сохраняет product-authored Task/Attempt history поверх copied reports tree и не мутирует backend baseline; backend cycle admits new Task/Attempt through the public API, while the snapshot frontend harness never synthesizes or rewrites identity and Playwright выбирает ровно один completed public Task/Attempt по exact snapshot run instead of starting a second provider-backed init; missing/ambiguous identity is a fail-closed frontend evidence error; `snapshot_reports_missing` после terminal backend failure записывается как dependent skipped frontend status, а не independent frontend regression; shell tests закрепляют, что failed headless refresh row remains ineligible for frontend smoke even if a triage snapshot directory exists
  ...
  - W24G entry evidence is retained in `fixtures/conformance/w24g-entry-metric.json` and verified by `internal/conformance`: first-pass success is 100%, otherwise-valid repair entry is 0%, p95 provider starts is 2, so the conditional envelope-reduction slice is deferred rather than inferred from negative incident cases.
...
- `scripts/frontend-live-e2e.sh` и `npm run e2e:live --prefix ui` используют Playwright:
  ...
  - release-facing `init-inspect` validates the current ProductShell over the copied backend refresh snapshot: contextual Setup, `Tasks / Architecture / Changes`, exact Task/Attempt Pipeline Studio and public runtime evidence, run deep-link reload/Back restoration, snapshot-isolated Evidence/Publish and global Ask citation return
