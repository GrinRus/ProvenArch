...
## Consequences
Workspace manifests with `repos[].analysis.role` are invalid. Collect shard manifests using legacy aliases such as `covered_topics`, `question`, `relation`, `source`, `target`, finding `summary`/`inference`, `evidence_citation_ids`, top-level `step_contract`, or `compatibility` fail through schema/contract validation. Existing valid all-repo runtime behavior is unchanged.
