...
- для `init.step1.collect` и `refresh.step1.collect` collect prompt начинает task-specific surface с `FIRST COLLECT ARTIFACT PAIR COMMAND` сразу после provider identity: suggested authored doc path + literal task-specific manifest skeleton должны быть записаны как focused marker-free validation-ready pair до broad second-pass repository sweep. До этой команды provider не должен вызывать `read_file`, `list_directory`, `grep_search`, `glob` или repo exploration; repo entrypoint hints в collect prompt применяются только после того, как first artifact pair уже существует. Generic manifest examples не добавляются в collect prompts; task-specific skeleton является единственным JSON template, а поздний docs-first block не дублирует heredoc command. Normal first-action pair является минимальным validation-ready scoped evidence surface: normal prompt требует targeted evidence pass после heredoc и перезапись authored doc + manifest более богатым evidence-backed content там, где repo evidence это поддерживает, но marker-free валидная пара больше не forced-degrades в recovery-only docs. Collect artifact monitor использует extended enrichment window для collect partial/post-artifact stalls перед repair; если provider stalls после валидной marker-free пары, validation может завершить shard без pair-recovery. Referenced authored docs с legacy marker/scaffold prose reject-ятся collect validation-ом и идут в collect pair-recovery; неизменённая legacy bootstrap pair также считается `artifact_quality:*` blocker. Общий runtime может сделать одну collect pair-recovery попытку, если provider оставил stdout/stderr diagnostics, не записал authored artifacts или оставил bootstrap-only authored doc; manifest-only repair разрешён только для non-bootstrap authored docs с missing/invalid `write_root/shard-pack-manifest.json`. Pair recovery пишет только suggested doc + manifest как marker-free validation-ready recovery fallback с scoped path evidence и recovery semantic finding; targeted evidence pass after first write is opportunistic, если provider продолжает выполнение. Marker-bearing docs и marker-free `Recovery Summary`/`Recovery Bootstrap` scaffold reject-ятся collect validation-ом. Manifest-only repair пишет только `shard-pack-manifest.json` и начинается с `FIRST COLLECT MANIFEST REPAIR COMMAND` plus absolute heredoc write target вокруг exact skeleton. Repair prompts instruct overwrite for invalid manifests rather than read/diff/patch, не повторяют validation-error field cues, а engine write-set guards запрещают лишние writes.
...
- `init-inspect` обязан проверить readability выбранных Review artifacts через UI и `/api/artifacts`: markdown preview не должен быть placeholder/scaffold, C4 raw Mermaid не должен быть gap-only, а screenshots должны подтверждать читаемый desktop/mobile Review surface.
