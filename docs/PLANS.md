...
4) Add prompt snapshot/contract tests that verify collect first-action commands write marker-free seed artifact pairs, unchanged collect seeds/recovery fallbacks are rejected before apply, draft first-action commands remain bootstrap-only until enriched, and final success is forbidden from generic placeholder text.
5) Tighten runtime prompt policy in `steppolicy`/`promptcontract`: collect first-action pairs are seed-only write surfaces and must be overwritten with evidence-backed content before success, draft first-action skeletons may bootstrap required files, and single-repo validator prompts must not return `PASS` with zero findings/questions when coverage gaps remain.
