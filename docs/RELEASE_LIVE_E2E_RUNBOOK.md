...
- Если authored docs отсутствуют и provider был полностью silent, `qwen-code` делает один bounded fresh retry с warning telemetry; exhausted silent retry остаётся `runner_unavailable`. `claude-code` сохраняет zero-output fail-fast для collect/as-is/non-scoped steps, но на validator/proposals steps (`init|refresh.step3.findings`, `init|refresh.step4.proposals`) первый fully silent pre-artifact stall получает один bounded retry; exhausted scoped silence остаётся `runner_unavailable`. Partial authored artifacts у всех providers остаются `runtime_contract_failed`, если manifest/draft/verdict невалиден после focused repair.
