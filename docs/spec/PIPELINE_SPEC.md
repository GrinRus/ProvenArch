...
- provider-free audit does not replace step3 validation and never repairs artifacts;
- selected-run mode requires the exact matching final/citation indexes and validator `PASS`, then
  checks contained inventory, reciprocal references, concrete repository evidence and Architecture
  Home/content invariants;
- promoted-current mode additionally compares each promoted canonical document digest with its
...
- Runtime write audit is an execution contract in live/headless mode: only the explicit provider `write_root`/`draft_final_root` and orchestrator-owned run log/history roots are allowlisted; mutations of protected workspace surfaces, any other unclassified workspace path, or analyzed repo working trees are surfaced as run warnings/logs (`runtime_write_audit_unexpected_mutation`, `runtime_write_audit_repo_skipped`) and turn an otherwise-successful provider step into `runtime_contract_failed`. An incomplete filesystem/repo snapshot is reported as `runtime_write_audit_failed` and fails closed before provider execution or promotion. Protected workspace mutations are restored after the provider exits when the post-run fingerprint still matches, while a concurrent edit is never overwritten and is reported as `runtime_write_audit_restore_conflict`; audited provider calls are serialized within a run so rollback cannot reintroduce another task's transient mutation. Protected/unclassified workspace snapshots and restore use one descriptor-backed root handle per check/use sequence, so a symlink swap or outside-root replacement fails closed without mutating the outside path. Read-only isolated repo clones use a lightweight HEAD/index/mode integrity snapshot with `GIT_OPTIONAL_LOCKS=0` instead of writable `git status`, while writable repos still use porcelain status; if a repo status/snapshot cannot be read after runtime, ACP reports `runtime_write_audit_repo_skipped` with `status_unavailable_after_runtime` and also fails the step because the read input can no longer be trusted.
...
Provider model/effort is an optional provider-scoped runtime profile. `runtime.profile.providers`
is resolved as `provider-specific env > workspace profile > provider native default`; when the last
source wins, ACP omits model/effort CLI arguments and reports `provider_default`. The resolved
profile is captured at run admission and passed to every task using that provider, so later env or
manifest changes cannot alter a queued/running run. The model identifier is opaque; effort values
are validated against adapter capabilities (`claude-code`: low/medium/high/max, `codex-code`:
none/low/medium/high/xhigh/max, `qwen-code`: unsupported).
...
- `validator-verdict.json.questions[]` items use required `id/text` and optional `priority/related_ids` only; `citation_ids` and other additional question fields are contract-invalid. Citation identity belongs in canonical finding provenance/evidence.
- `step3` final `PASS/FAIL` принадлежит orchestrator-owned `effective-verdict.json`: deterministic technical checks и provider-free audit проверяют staged artifact integrity и contract correctness (indexes, references, canonical documents, required artifact shape). Provider-authored `validator-verdict.json` остаётся immutable draft evidence после admission/consistency validation. Source-repository security/privacy/compliance/architecture-risk/operational observations записываются как advisory findings/questions; unmatched provider issues сохраняются как advisory warnings и не меняют effective technical verdict. Deterministic document/path/citation errors и audit errors остаются blocking независимо от provider `PASS/FAIL` prose.
...
Run lifecycle registry не публикует queued/running/terminal candidate до успешной atomic записи
primary `reports/taskruns/run-history.json`. Queue replacement сохраняет new queued + old
superseded records одной transaction; API/list/history boundaries возвращают deep copies.
Primary persistence failure оставляет последний durable registry snapshot и bounded
`history_diagnostics` в ответе `GET /api/pipeline/runs`; отдельный `.last-good` failure не
откатывает уже committed current history. User/shutdown cancellation использует terminal `status=canceled`,
`error_code=run_canceled`; stale restart reconciliation остаётся `failed`,
...
- Runtime converts validation failures once at the boundary into a deterministic internal issue set
  with stable code/class/path fields. Collect and draft recovery selection consumes that set, not
  message fragments. Class-specific transitions reject mixed issue classes where required, name an
  exact target stage and have a one-attempt budget; a repeated/no-op stage terminates as
  `runtime_contract_failed`. Equivalent ordered or paraphrased provider diagnostics therefore route
  identically for Claude/Qwen/Codex.
...
- The bounded Qwen collect prompt treats the shard manifest root as closed: claim identities exist only in `citations[].claim_ids`, never in top-level `claims` or `claim_map`. Its compact markdown contract explicitly bans the validator process phrases `bounded read`, `bounded evidence read`, `bounded collection`, `bounded pass`, guessed path/file/evidence and missing expected evidence. Every compact semantic provenance object uses exact `kind: observation`; unsupported inferences are omitted and represented as observed coverage gaps instead of lexical aliases such as `inferred`. If strict validation reports exactly one known forbidden wrapper at the root `additionalProperties` rule, Qwen manifest repair MAY use a compact one-read/one-write contract that removes only that diagnosed root field and preserves every canonical nested value. Nested or unknown additional properties MUST remain on the full fail-closed provider repair path, and complete backend validation is still required after the rewrite.
...
- Normal `step0.constitution`, `step2.asis_docs`, and `step4.proposals` prompts carry a same-turn completion requirement. For step0, the provider writes the initial draft set and immediately rewrites `charter-overview.md` from repository-entrypoint evidence before success. For step2, the first bounded action sequence allows at most one evidence read/list command, then requires one mechanically simple direct-literal shell command that writes validation-ready `overview.md`, `summary.md`, and `architect-summary.md` with single-quoted heredocs, performs the marker check, and writes `asis-draft-manifest.json` last. Python/Node/Ruby/Perl/awk/jq inline generators, template programs, generated source strings and nested quote tricks are prohibited until the full step2 write set exists. Step4 retains its evidence-first work unit and manifest-last ordering. The first provider item must be command execution rather than assistant/status prose. Headless draft adapters use bounded first-command pre-artifact windows: Claude `step2.asis_docs` uses a 5-minute initial/retry window because its largest prompt can spend several minutes reasoning before the first write; other normal draft steps retain the shared 180s window. Tripping that window is still runtime pressure, not an accepted success path. A successful normal happy path should not require focused repair/stall pressure.
- `runtime_quality.stall_pressure` describes actual runtime pressure before a valid artifact exists or while artifacts are invalid/no-fresh/retry/analysis-only. A provider lifecycle stall after `artifact_state=valid` / `artifact_valid=true` and without validation error is recorded as additive `valid_artifact_controlled_stop(s)` telemetry, not as `stall_pressure`, and does not block `Excellent` by itself. The paired `retry scheduled / terminate_and_validate` diagnostic carries the same outcome through `manifest_state`; `manifest_state=valid` is part of that single controlled-stop lifecycle and must not be counted again as an actual stall, while missing/invalid state remains stall pressure.
...
- Specialized semantic cleanup retries are selected only for their complete validation class. In particular, `draft_artifact_enrichment_downstream_index_retry` is allowed only when every reported draft problem is a stale downstream-index availability or unvalidated zero-document claim. A mixed result that also contains the step2 Architecture Home process-narration error uses the existing provider-authored `draft_artifact_enrichment_architecture_home_cleanup` path first; the full draft contract remains authoritative and any remaining downstream claim is still rejected. This routing never edits or synthesizes markdown in ACP.
...
- Process-contaminated collect markdown is also contract-invalid: final collect docs must not narrate bounded reads/passes, guessed paths/files/evidence, expected-missing concrete path checks, recovery attempts, or later repair. Missing expected concrete files such as `src/foo/Bar.java path was not present` are process contamination, and wording like `not examined in this bounded pass` is still runtime-process narration; write operator-facing gaps as `not confirmed in scoped repository evidence` or as a concrete missing evidence category. A decision-ready coverage gap that a named spec/scope was not observed is valid when it does not cite guessed evidence. Collect-authored markdown must also never publish internal execution paths such as `.acp/repos/`, `reports/taskruns/`, `staging/final/`, `staging/shards/`, `write_root`, or `draft_final_root`; use stable `repo:path` references or canonical report paths instead. When existing authored markdown has this contamination, runtime routes to provider-authored collect pair repair with mandatory fresh rewrite of the same markdown target; manifest-only runtime recovery must not turn that document into hidden deterministic success.
...
- normal и focused repair prompts для `step2.asis_docs` начинают task surface с `FIRST AS-IS DRAFT COMMAND`; normal path допускает один bounded evidence read/list command, после которого direct-literal heredoc command обязан создать все три referenced draft files в `draft_final_root`, проверить marker contract и только затем создать `asis-draft-manifest.json` в `write_root`, до broad as-is assembly
...
- Pair validation also binds both indexes, `citation_index_path` and every staged document to the
  active run; every citation must resolve to a concrete regular file inside its resolved repository
  root after symlink evaluation. Unknown repositories, missing files, traversal, symlink escape and
  foreign-run paths are hard failures. Key Architecture Home/findings/proposal documents that claim
  complete citation coverage with no citation ids are rejected.
- A selective refresh may copy immutable baseline document bytes only after its current
  document/citation binding is still reciprocal and no citation or referenced document was removed.
...
- `validator-verdict.json` обязан содержать canonical metadata trio `version=1`, `run_id`, `generated_at` вместе с `verdict` и `checked_paths`; provider CLI может вернуть absolute `checked_paths` или `issues[].path`, но admission принимает их только при symlink-resolved containment внутри selected workspace, нормализует в workspace-relative logical paths, а затем выполняет selected-run inventory checks
...
only for silent/unavailable execution. For draft steps, when both the normal invocation and its
transport retry stall silently before any artifact is written, the final budget slot is another
full provider-authored invocation; a focused draft repair is reserved for runs that already have
real artifact progress. Deterministic repairs do not consume provider starts but
