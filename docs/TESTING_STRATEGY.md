...
- strict collect validation:
  ...
  - collect pair recovery запускается один раз при no authored artifacts + non-empty provider diagnostics или exhausted fully silent collect fresh retry, разрешает писать только suggested authored doc + `shard-pack-manifest.json`; prompt является evidence-first final repair без heredoc seed/fallback, исключает large/generated/lock/test-duration evidence, требует manifest write immediately after markdown write, а `Recovery Summary`/`Recovery Bootstrap`/`Recovery Evidence Summary` scaffold reject-ится
