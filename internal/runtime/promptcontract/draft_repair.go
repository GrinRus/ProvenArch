...
func ComposeDraftArtifactEnrichmentPrompt(provider acpruntime.Provider, task acpruntime.Task, validationErr error) string {
	...
	case "init.step2.asis_docs", "refresh.step2.asis_docs":
		lines = append(lines,
			...
			"- In overview.md, never mention run IDs, pipeline step names, collect passes, repair/enrichment attempts, or how runtime assembled the document; state repository facts and evidence gaps directly.",
