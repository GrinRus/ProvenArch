...
2) MVP scope фиксирован: local-first как основной режим + headless multi-provider runtime (`claude-code` default, `qwen-code` optional, `codex-code` release peer) при обязательном deterministic baseline `--runtime fake` для required CI.
