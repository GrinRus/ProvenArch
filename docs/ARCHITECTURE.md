...
3) **Orchestrator (`internal/orchestrator`)** *(implemented baseline)*
   ...
   - Collect recovery имеет два слоя: non-silent no-artifact collect может выполнить одну provider-authored pair-recovery попытку (`suggested overview doc + shard-pack-manifest.json`), а collect с non-bootstrap authored docs — одну manifest-only provider repair попытку; если этот manifest-only repair exhausted/stalled или завершился без валидного manifest, runtime может выполнить deterministic manifest recovery только из уже provider-authored markdown docs и bounded `repo/path` evidence, записав ровно `write_root/shard-pack-manifest.json` с diagnostic `collect_manifest_runtime_recovery`. Unchanged seed/bootstrap authored docs не маскируются repair success; write-set guards запрещают любые лишние файлы
...
5) **Runtime providers (`internal/runtime/*`)** *(implemented baseline)*
   ...
   - collect prompt начинает normal `init.step1.collect` / `refresh.step1.collect` с `COLLECT EVIDENCE-FIRST ARTIFACT PAIR`: provider сначала делает bounded evidence pass по repo entrypoint hints и assigned `path_scopes`, не читает `reports/taskruns/**`, raw logs, sibling shards или archive docs, затем пишет marker-free evidence-backed authored doc + `shard-pack-manifest.json`. Task-specific manifest skeleton остаётся в prompt как schema/key/type guide, но copying unchanged skeleton invalid; `ValidateCollectManifestInRoot` reject-ит bootstrap-only docs/recovery fallback docs и scaffold-only semantic (`contains`-only repo/shard + generic owner-gap). Root-file shards читают только перечисленные root files first и пишут evidence-backed root overview без recursive sweep. Для `init.step1.collect` / `refresh.step1.collect` runtime выполняет максимум одну collect pair-recovery попытку только если provider оставил stdout/stderr diagnostics и не записал authored artifacts, и максимум одну manifest-only repair попытку, если provider уже записал non-bootstrap authored docs в `write_root`, но `shard-pack-manifest.json` отсутствует или невалиден; manifest-only repair начинается с `COLLECT MANIFEST EVIDENCE-FIRST REPAIR` и `FIRST COLLECT MANIFEST REPAIR COMMAND`, который provider выполняет как next filesystem action. Command читает existing authored markdown under `write_root`, derives documents/citations/entities/edges/findings/coverage/questions from that content plus listed repo evidence candidates, writes only `write_root/shard-pack-manifest.json`, uses the task-specific skeleton only as a schema guide, avoids copied placeholders and citations-only generic semantic, applies the same root-file primary evidence preference, instructs overwrite instead of read/diff/patch invalid manifests, не повторяет validation-error field cues. Если provider repair exhausted/stalled или вышел без валидного manifest, shared engine применяет deterministic `collect_manifest_runtime_recovery`: читает только existing non-bootstrap authored markdown under `write_root`, строит manifest с non-container `documents` edges, runtime-recovery finding/question/coverage note и bounded repo/path evidence, пишет только `write_root/shard-pack-manifest.json`, затем снова запускает strict collect validation. Engine write-set guards разрешают только expected pair или `write_root/shard-pack-manifest.json`
   ...
   - focused draft repair может писать только step manifest в `write_root` и draft files под `draft_final_root`; repair prompt начинается с command-first heredoc write-set для manifest + referenced draft files, задаёт exact absolute `write_root`/`draft_root` один раз и дальше использует только `"$write_root/..."` / `"$draft_root/..."` targets, чтобы provider не перепечатывал длинные slash-separated пути вручную; writes outside this set остаются `runtime_contract_failed`, а activity monitor учитывает draft files рекурсивно внутри `draft_final_root`
   - `init|refresh.step2.asis_docs` использует strict shared draft contract: `step_contract="as_is"`, required canonical outputs `reports/as-is/overview.md`, `reports/coverage/summary.md`, `reports/agent-outputs/architect/summary.md`, а extra outputs разрешены только под `reports/as-is/<domain>/overview.md`; normal и repair prompts начинают as-is surface с единственного `FIRST AS-IS DRAFT COMMAND`, который пишет `asis-draft-manifest.json` в `write_root` и `overview.md`/`summary.md`/`architect-summary.md` под `draft_final_root`
