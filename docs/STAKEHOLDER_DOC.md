...
| Task-first UI, runner presets and content-aware artifact workbenches | planned (Epic 23) | [`UI_TASK_FIRST_PRODUCT_DESIGN.md`](UI_TASK_FIRST_PRODUCT_DESIGN.md), [`UI_TASK_FIRST_PRODUCT_MIGRATION_PLAN.md`](UI_TASK_FIRST_PRODUCT_MIGRATION_PLAN.md), `docs/assets/ui-task-first-product/*.png`; no current API/UI implementation is claimed. |
| Weak-model validation and promotion hardening | planned (Epic 24) | Provider-free task-bound verdict, evidence, graph, pre-promotion audit, effective technical verdict and bounded recovery work; implementation has not started. |
| Task-first live E2E and hardened runtime evidence alignment | planned (Epic 25) | Cross-epic release-gate task after `23O`/`24I`: migrate public `init-inspect` to Task/Attempt flow and consume stable public audit/verdict/budget evidence without changing canonical profile taxonomy. |
