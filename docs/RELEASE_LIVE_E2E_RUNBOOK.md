...
- `shard-pack-manifest.json.citations[].repo/path` и `semantic.*[].provenance.evidence[].repo/path` должны resolve-иться к текущему repo scope/root и реально существующему relative file path under that repo root; directory-only refs are invalid evidence even when the directory is the assigned shard scope. Guessed evidence paths such as Maven `pom.xml` in a Gradle-only repo are `runtime_contract_failed` / focused-repair input at collect time, not a manual artifact-quality-only finding. Provider-authored collect/repair commands must prove citation/provenance paths with file-level checks (`test -f`, `rg --files`, or portable `find ... -type f -print`) before successful exit; `jq empty` / JSON parse success alone is insufficient.
...
- `collect_manifest_repair` больше не использует read-only preflight как первый action. Первый provider-authored filesystem command обязан прочитать bounded authored docs/evidence и записать `write_root/shard-pack-manifest.json` до возврата; preflight-only completion, status prose “I have enough evidence / I will write / I am replacing” без manifest write считается no-op repair и остается `runtime_contract_failed` unless runtime recovery explicitly succeeds.
...
- Step-level `Excellent Blockers By Step` diagnostics are derived only from public reports/logs/raw taskrun metadata. They explain which runtime step still needed repair/stall recovery, but they do not change the strict PASS/FAIL rule by themselves; `runtime_quality.*` pressure remains a label cap unless paired with a strict blocker. `valid_artifact_controlled_stop` rows may appear as diagnostic context, but are not `Excellent` blockers unless paired with actual repair/stall pressure.
