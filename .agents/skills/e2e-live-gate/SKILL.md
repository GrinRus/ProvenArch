...
## Operational rules
1) Release gate запускать только на trusted машине, где доступны canonical absolute `path` checkout'ы из curated presets.
...
## Common blockers
- `repos[1] path does not exist: /absolute/path/to/...`
  Причина: path profiles не подготовлены.
- `mkdir: /absolute: Read-only file system`
  Причина: хост не подходит для canonical curated path layout.
