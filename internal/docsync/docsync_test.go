...
func TestMultiRepoE2EDocsAreConsistent(t *testing.T) {
	...
	required := map[string][]string{
		".agents/skills/e2e-live-gate/SKILL.md": {
			...
			"operator/SWE-agent writes the reasoning layer",
		...
		"docs/RELEASE_LIVE_E2E_RUNBOOK.md": {
			...
			"not GitHub Actions workflow",
			"Scripts produce setup/execution artifacts and machine verdicts only",
			...
			"frontend UI/API init inspection",
		...
		"docs/TESTING_STRATEGY.md": {
			...
			"Scripts produce machine evidence and verifier-backed verdicts only",
