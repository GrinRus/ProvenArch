...
- generic `codex` plugin/Cloudflare/state-db noise (`chatgpt.com/backend-api/plugins/featured`, Cloudflare HTML, `failed to renew cache TTL`, `state db`, `Operation not permitted`, MCP stream/list 504) учитывать как secondary telemetry; сами по себе такие строки не должны поднимать `runner_unavailable`, если raw runtime/session-summary не зафиксировали terminal provider failure. Default `codex-code` headless invocation запускается с `--ignore-user-config` и `--ignore-rules`, чтобы пользовательские MCP/plugins/rules не были входом live runtime и не могли задержать artifact-only шаг до первого model action.
