...
- [ ] Remove the remaining `init.step2.asis_docs` first-pass missing-manifest/exact-shard-completeness/operator-summary repair path before Phase A can reach `Excellent`.
...
- [ ] Rerun diagnostic `smoke tiny codex-code` after Codex quota/auth is available again; current provider usage-limit blocker prevented reaching `step2`.
