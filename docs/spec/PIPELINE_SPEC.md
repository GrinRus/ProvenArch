...
- `draft_artifact_enrichment` для `step2` обязан читать текущий draft manifest, bounded current-taskrun staged evidence (`shard-pack-manifest.json` summaries, optional final/citation indexes and at most a small representative authored-doc sample) и fresh rewrite-нуть все три markdown targets (`overview.md`, `summary.md`, `architect-summary.md`) с repo/path evidence, coverage gaps и decision-ready operator summary; unchanged files, scaffold markers or context-overflow caused by broad workspace/repo reads remain runtime contract/recovery bugs, not artifact-quality-only warnings.
