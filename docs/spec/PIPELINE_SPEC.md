...
- `documents[].path` должен быть artifact-root-relative и должен resolve-иться в существующий provider-authored file under collect `write_root`; missing file references, directory references, absolute paths, workspace-level prefixes и duplicated `artifact_root` prefixes являются invalid collect artifacts, поэтому `step2` не должен первым обнаруживать broken document reference
