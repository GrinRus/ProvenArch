...
func DocFirstFilesystemPolicy(task acpruntime.Task) string {
	...
	case "init.step1.collect", "refresh.step1.collect":
		lines = append(lines,
			`- Do NOT delegate to agent/subagent helpers and do NOT use todo_write-style planning.`,
			`- The first collect filesystem action must be one mechanically simple bounded evidence-write action: read existing repo entrypoint hints / assigned path_scopes and write the authored document plus shard-pack-manifest.json before returning.`,
			`- Cap that first action to at most 8 representative files and at most the first 6000 bytes from each file; oversized files are truncated or skipped while the action continues.`,
			`- Do not run analysis-only narration, status/progress text, todo/planning, read-only preflight, or broad repository sweeps before that first evidence-write action.`,
			`- Produce runtime-authored documents in write_root and then write shard-pack-manifest.json in write_root.`,
			`- Evidence-write pair requirement: the first action writes the suggested overview doc and shard-pack-manifest.json as one focused marker-free evidence-backed artifact pair.`,
...
func CollectFirstActionSection(task acpruntime.Task) string {
	...
	return strings.Join([]string{
		...
		"- The next filesystem action must be one mechanically simple action that reads bounded evidence and writes both exact targets before returning.",
		...
		"- Do not emit analysis-only prose, status/progress narration, todo/planning, broad repo sweeps, or a read-only preflight before writing the artifact pair.",
		"- Do not use generated source-code strings, Python f-strings, .format templates, or nested quote tricks for markdown/JSON assembly.",
		"COLLECT FINAL WRITE REQUIREMENT:",
		"- Write the authored document and shard-pack-manifest.json with evidence-backed content from the bounded reads in the first action.",
