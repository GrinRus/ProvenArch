...
- `codex-code` collect steps use a 3-minute initial pre-artifact stall window. This is provider policy, not matrix timeout override: if no authored collect artifacts appear within that window, the step still follows the normal stall/focused-repair classification path.
...
- Process-contaminated collect markdown is also contract-invalid: final collect docs must not narrate bounded reads/passes, guessed paths/files/evidence, expected-missing concrete path checks, recovery attempts, or later repair. Missing expected concrete files such as `src/foo/Bar.java path was not present` are process contamination, but a decision-ready coverage gap that a named spec/scope was not observed is not. When existing authored markdown has this contamination, runtime routes to provider-authored collect pair repair with mandatory fresh rewrite of the same markdown target; manifest-only runtime recovery must not turn that document into hidden deterministic success.
