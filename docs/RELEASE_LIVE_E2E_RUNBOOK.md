...
- Если `draft_artifact_enrichment` вернулся или был controlled-stop/stalled без fresh valid rewrite, а validation всё ещё фиксирует `draft_artifact_enrichment_noop_or_scaffold`, runtime допускает один compact provider-authored retry `draft_artifact_enrichment_no_action_retry`. Prompt intentionally shorter than the normal enrichment prompt: next action must be one `python3` filesystem command that reads bounded current-run evidence and overwrites every referenced markdown target before returning. Для `step0` retry читает только bounded repo entrypoint evidence и не должен упоминать downstream evidence; для `step4` retry должен выполнить не больше одного bounded evidence listing/read pass before writing both `proposal.md` and `changelog.md`, а при sparse proposal evidence записать decision-ready `no-actionable-proposal` gap. Повторный no-op/scaffold/invalid result remains terminal `runtime_contract_failed`; ACP не синтезирует proposal/as-is/constitution markdown вместо provider. Focused draft enrichment uses a minimum 3-minute pre-artifact wall-clock/stall window so heavy current-run evidence does not get cut off before the first provider-authored mutation.
