...
func AsIsFirstActionSection(task acpruntime.Task) string {
	...
	lines := []string{
		...
		"- Treat this as a write-time contract, not a repair hint: the same first filesystem command must scan all three completed markdown targets for those forbidden markers before it writes asis-draft-manifest.json, and must rewrite any offending sentence as an architecture fact, coverage gap, or operator decision in that command.",
