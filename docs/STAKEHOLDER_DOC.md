...
| Console evidence trust and IA reset | in progress (truth + shell slices) | The native History shell now provides `Tasks / Architecture / Changes` plus first-class `/settings`; Documents is the default Architecture mode with Diagrams/Model/Findings siblings, while the explicit `/tasks/legacy` read-only migration surface, contextual evidence, deep URL context, run-pinned review, server-authored coordination/runtime identity, safe Git publication, responsive navigation/context drawer, current-workspace authority and global read-only Ask remain preserved. |
| Task-first UI, runner presets and content-aware artifact workbenches | W23B1–W23N additive route/composer/Inbox/outcome/Pipeline Studio/Architecture-context/Markdown/model/Evidence/review/Changes/Ask/settings/state-quality foundation implemented; Tasks-primary nav and W23O legacy route closure landed (Epic 23 UI remains deterministic-DoD tracked) | [`spec/TASK_SPEC.md`](spec/TASK_SPEC.md), [`UI_TASK_FIRST_PRODUCT_DESIGN.md`](spec/UI_TASK_FIRST_PRODUCT_DESIGN.md), [`UI_TASK_FIRST_PRODUCT_MIGRATION_PLAN.md`](spec/UI_TASK_FIRST_PRODUCT_MIGRATION_PLAN.md) and 2026-08-11 Task/Attempt ADRs fix the target identity/persistence/admission/publication boundary; typed Task/Attempt routes, a scope/runner-aware New Task composer with Attempt admission, public Inbox/detail history, exact-run semantic outcome, Attempt-bound Pipeline Studio, explicit Task-scoped current Architecture, allowlisted Markdown reader/editor, read-only schema-aware entity/edge inspector, non-authoritative Mermaid Evidence Studio, filtered findings/question/gap queue, exact Task-scoped Changes context, explicit Ask/Runner authority boundaries, Task state/accessibility coverage and a primary nav limited to Tasks/Architecture/Changes now exist without legacy-run fallback. Pre-Task runs remain only under explicit read-only `/tasks/legacy` migration routes. |
...
| Task-first live E2E and hardened runtime evidence alignment | 25A/25B provider-free implementation in progress; trusted gate pending | Cross-epic release-gate task after `23O`/`24I`: `init-inspect` now resolves exact public Task/Attempt/run identity and batch reports consume public audit/verdict/budget fields only. Snapshot mode fails closed when product-authored Task/Attempt history is missing or ambiguous; no synthetic legacy identity or second analysis is allowed. |
...
- planned: Epic 23, Epic 24 and cross-epic release-gate alignment Epic 25 (25A/25B implementation in progress)
