...
func TestAsIsFirstActionSectionWritesEvidenceBackedDraftSetFirst(t *testing.T) {
	...
	required := []string{
		...
		`Architecture Home must never reference reports/taskruns/**, taskrun staging paths, write_root, draft_final_root, or raw runtime artifacts`,
		`Architecture Home must not mention run IDs, pipeline step names, collect passes, repair/enrichment attempts`,
