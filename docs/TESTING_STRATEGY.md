...
- strict collect validation:
  ...
  - collect pair recovery запускается один раз при no authored artifacts + non-empty provider diagnostics или bootstrap-only authored docs, разрешает писать только suggested authored doc + `shard-pack-manifest.json`; first recovery write должен быть marker-free validation-ready fallback с scoped path evidence и recovery semantic finding, а `Recovery Summary`/`Recovery Bootstrap` scaffold reject-ится
