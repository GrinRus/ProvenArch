...
3) **Orchestrator (`internal/orchestrator`)** *(implemented baseline)*
   ...
   - generic `codex` plugin/Cloudflare/state-db warnings (`plugins/featured`, Cloudflare HTML, cache/state-db permission noise) считаются secondary telemetry и не должны сами поднимать `runner_unavailable`; default `codex-code` headless invocation also uses `--ignore-user-config` + `--ignore-rules`, so user MCP/plugin/rule surfaces cannot stall ACP artifact-only tasks before model actions
