...
- Если Qwen focused `collect_pair_repair` исчерпал pre-artifact wall-clock с активным stdout stream, но без единого authored file, shared engine завершает этот процесс и делает одну retry с compact stall-focused write-first prompt. Retry не расширяет canonical timeout profile и не считается success без валидных markdown + manifest; повторный stream-only stall остаётся `runtime_contract_failed` и останавливает release sequence.
