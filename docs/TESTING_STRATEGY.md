...
- draft enrichment regression tests also cover one provider-authored no-action retry: if enrichment leaves bootstrap/noop markdown and validation reports `draft_artifact_enrichment_noop_or_scaffold`, runtime may do one compact `draft_artifact_enrichment_no_action_retry` whose prompt requires exactly one `python3` filesystem command to read bounded current-run evidence and overwrite every markdown target. For `init.step0.constitution`, the retry names the exact `charter-overview.md` target and bounded repo entrypoint candidates, and prompt/validation tests reject downstream/final/shard/validator/proposal/coverage/taskrun/runtime-provider leakage in constitution markdown. For `step4`, the retry requires at most one bounded evidence listing/read pass before writing both `proposal.md` and `changelog.md`, including a decision-ready no-actionable-proposal gap when evidence is sparse. A second no-op/scaffold result remains terminal `runtime_contract_failed`, and successful retry must not emit exhausted repair telemetry.
- draft enrichment regression tests also cover printed-command no-action retry output: if `draft_artifact_enrichment_no_action_retry` prints shell/Python command text such as `python3 - <<...` without file mutation, runtime may do one `draft_artifact_enrichment_command_text_retry`; success still requires provider-authored fresh rewrites, and repeated printed-command/noop/scaffold output remains terminal.
...
- `scripts/full-run-batch.sh` — canonical live batch + frontend live e2e:
  ...
  - DoD/UI precheck failures generate execution evidence before runtime starts. `npm ci --prefix ui` and `playwright install chromium` are bounded by `ACP_LIVE_PRECHECK_UI_TIMEOUT_SEC`; script tests cover a hung npm command producing `precheck_failed` plus `[precheck-timeout]` log evidence, not a silent running batch.
