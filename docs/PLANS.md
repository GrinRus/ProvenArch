...
- [ ] Rerun diagnostic `smoke tiny` on `codex-code` after the collect manifest shape-cleanup slice; continue only if the result is `strict PASS`, `artifact_quality_status=passed`, no actual `runtime_quality.repair_heavy`, no actual `runtime_quality.stall_pressure`, and verdict `Excellent`.
