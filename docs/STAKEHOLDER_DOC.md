...
| Console evidence trust and IA reset | done (Epic 20, usability hardening) | The native History shell provides `Home / Runs / Knowledge / Changes`, distinct Runs history and Run Studio routes, contextual evidence inside a four-mode Change Review, deep URL context, run-pinned evidence, server-authored coordination/runtime identity, safe Git publication, responsive navigation/context drawer, current-workspace Knowledge and global read-only Ask. |
...
| Outcome-first Architecture and recovery UX | implemented | Home/Architecture expose the promoted result and interactive C4 projection; Runs exposes structured useful progress, outcome/recovery and audited child-run retry with validator-gated promotion. |
