...
func ComposeArtifactOnlyPrompt(provider acpruntime.Provider, task acpruntime.Task) string {
	...
	if firstAction := strings.TrimSpace(steppolicy.AsIsFirstActionSection(task)); firstAction != "" {
		sections = append(sections, firstAction)
	}
	...
	if firstAction := strings.TrimSpace(steppolicy.QAFirstActionSection(task)); firstAction != "" {
		sections = append(sections, firstAction)
	}
