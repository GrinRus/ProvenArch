...
- 2026-06-09: Follow-up diagnostic (`regres-long-posthog-ftgo-20260609T042521Z`) showed the command-shaped heredoc template was still insufficient: Claude wrote `root-overview.md`, manifest-only repair received a 17KB prompt with the write shape, then stalled 180s with zero stdout/stderr and no `shard-pack-manifest.json` (`runtime_contract_failed`). Root cause: the prompt still required provider-side manual filling of a placeholder heredoc instead of an exact first filesystem command. The next fix makes manifest-only repair command-first: `FIRST COLLECT MANIFEST REPAIR COMMAND` reads existing authored markdown, derives documents/citations/entities/edges/findings/coverage/questions from that content and bounded repo evidence candidates, writes only `shard-pack-manifest.json`, and remains under the existing write-set/validation gates.
- 2026-06-09: Follow-up diagnostic (`regres-long-posthog-ftgo-20260609T045920Z`) showed command-first prompt text still did not force live Claude repair execution: `init.step1.collect` wrote a readable PostHog `root-overview.md`, manifest-only repair received a 27KB prompt with exact first command, then stalled 180s with zero stdout/stderr and no `shard-pack-manifest.json` (`runtime_contract_failed`). Root cause: contract completion still depended on provider choosing to execute repair instructions after authored docs already existed. The next fix adds shared deterministic `collect_manifest_runtime_recovery` after exhausted/stalled manifest-only repair: it reads only provider-authored markdown in `write_root`, derives a manifest with non-container semantic edges plus explicit runtime-recovery finding/question/coverage note, writes only `shard-pack-manifest.json` under the existing write-set guard, and leaves final acceptance to strict validation, `artifact_quality:*` gates and fresh live operator review.
- 2026-06-09: Follow-up diagnostic (`regres-long-posthog-ftgo-20260609T054619Z`) proved deterministic recovery removes the hard contract failure, but the live order was still wasteful: Claude wrote readable `root-overview.md` and `bin-overview.md`, then each missing manifest path waited through a 180s manifest-only repair with zero stdout/stderr before runtime recovery created a valid semantic manifest. Root cause: missing-manifest recovery still tried provider repair before the deterministic manifest builder even when non-bootstrap authored docs already existed. The next fix makes missing collect manifests runtime-recovery-first, keeps provider manifest-only repair for invalid existing manifests or failed runtime recovery, and preserves write-set/strict validation gates.
- 2026-06-09: Medium Claude diagnostic with extended provider activity windows proved the timeout override slice works: the first PostHog shard needed about 342s after artifact creation and then produced a valid, concrete manifest. The next defect appeared on `posthog-cli-common`: Claude wrote a rich markdown report, but the manifest semantic collapsed to a scaffold-only repo/shard wrapper with owner-gap finding, then provider manifest repair stalled instead of executing the embedded repair command. The next fix routes scaffold-only semantic manifest failures through deterministic `collect_manifest_runtime_recovery` before provider repair and strengthens recovered manifests with concrete service/component/datastore entities plus usage/dependency/configuration edges from authored markdown.
- 2026-06-09: Partial follow-up diagnostic (`regres-long-claude-posthog-ftgo-20260609T071059Z`, stopped before completion) showed the operator-requested preflight timeout increase did not affect runtime collect activity windows: Claude still hit 180s pre-artifact stalls and 90s post-/partial-artifact stalls, while some retries later produced valid authored docs/manifests. The next fix adds diagnostic-only provider activity timeout env overrides (`ACP_PROVIDER_*_ARTIFACT_STALL_SEC`, `ACP_PROVIDER_VALID_ARTIFACT_STOP_SEC`) that flow through matrix timeout env handling and remain blocked in release mode.
- 2026-06-09: Claude-only medium diagnostic with extended live activity windows (`regres-long-claude-posthog-ftgo-20260609T085556Z`) proved PostHog artifacts can now be substantive and frontend can complete, but FTGO failed quality on `artifact_quality.c4_context_scaffold_only`: C4 context stayed gap-only despite 84 model entities and 102 edges. Operator screenshot review also found Review/Publish preview readability was under-tested: DOM content existed, but the central preview could be visually blank or dominated by shell/log space. The current fix adds C4 context internal-relation fallback plus viewport-visible Review/Publish preview assertions and layout hardening before rerunning Claude/Codex.
- 2026-06-10: Claude medium diagnostic with provider stall windows raised to 900s proved old 300s limits still cut valid work (`as-is` completed in 332s, multiple collect shards exceeded 300s), but PostHog was blocked by a false `artifact_quality.placeholder_artifact` on `reports/changelog/runtime-proposals.md`: the changelog was evidence-backed and traceable, yet included the boilerplate note "Promote only after artifact validation succeeds." The next fix narrows placeholder detection so bootstrap/generic proposal text remains blocked while traceable finding/question-backed changelog content passes.
