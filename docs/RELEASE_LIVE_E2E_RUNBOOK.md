...
- frontend init-inspect budget берётся из effective runtime timeout profile/API (`ui_init_poll_timeout_sec`) и не расширяется автоматически до `pipeline_timeout+30s`. Diagnostic follow-pipeline режим допускается только при явном `UI_E2E_INIT_TIMEOUT_FOLLOW_PIPELINE=1`; `UI_E2E_INIT_TIMEOUT_CAP_SEC` может ограничить такой diagnostic override. Эти overrides не использовать как canonical release evidence.
