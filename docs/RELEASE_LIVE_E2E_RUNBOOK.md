...
- `session-summary.md` для timeout обязан показывать `pipeline_deadline_at`, `pipeline_timeout_elapsed_sec`, `deadline_missed_by_sec`, `last_progress_at`, `last_watchdog_tick_at`, `max_watchdog_tick_gap_sec` и `host_clock_gap_detected`. Если host sleep/clock jump detected, primary class остаётся `runtime_timeout`, а `infra_host_sleep_or_clock_jump_detected` считается contributing evidence;
