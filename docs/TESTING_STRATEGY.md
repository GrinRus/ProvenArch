...
- strict collect validation:
  ...
  - normal collect prompt требует, чтобы первый filesystem action одновременно читал bounded evidence и писал suggested authored doc + `shard-pack-manifest.json`; read-only preflight, analysis-only prose, broad sweep, f-string/`.format(...)` template writers и reliance on focused repair как штатный success path должны ловиться prompt contract tests
