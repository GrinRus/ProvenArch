...
func ComposeCollectArtifactPairRepairPrompt(provider acpruntime.Provider, task acpruntime.Task, validationErr error) string {
	...
	lines := []string{
		...
		"- The first command may read only read_context_roots and listed evidence candidates, at most 8 files and 6000 bytes per file; do not run open-ended find/grep/sed/tree sweeps.",
		...
		"- Before writing, the only allowed evidence prechecks are structural: exact targets stay under write_root, at least one listed evidence file was read, and each read file is within the size limit.",
