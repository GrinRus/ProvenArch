...
- draft enrichment regression tests also cover printed-command retry output: if provider-authored `draft_artifact_enrichment` prints shell/Python command text such as `python3 - <<...` without file mutation, runtime may do one `draft_artifact_enrichment_command_text_retry`; success still requires provider-authored fresh rewrites, and repeated printed-command/noop/scaffold output remains terminal. Normal, compact and command-text `step2.asis_docs` enrichment prompts are each contract-tested to require the exact eight non-empty Architecture Home sections; legacy generic `Architecture Surface` / `Evidence Used` / `Coverage Gaps` headings are not accepted substitutes. A live-observed Claude fixture also verifies that Architecture Home containing `reports/taskruns/**/staging/**` is rejected before promotion, while canonical artifact and concrete repository references remain valid.
