...
- draft enrichment regression tests cover the live-observed changed-error Architecture Home case: after every step2 markdown target is freshly rewritten, a remaining strict `runtime/process narration, manifest recap, or unsupported confidence` failure may schedule exactly one provider-authored `draft_artifact_enrichment_architecture_home_cleanup`. The retry is step2-only and non-recursive; it does not sanitize or accept content, and success still requires a fresh complete rewrite plus strict validation. The fixture includes the rejected sentence `scoped to the current run`.
