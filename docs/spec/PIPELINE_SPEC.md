...
- collect runtime до финальной validation может выполнить focused provider-authored recovery без ACP-side synthesis: если provider оставил stdout/stderr diagnostics, но не записал authored artifacts, общий engine делает одну collect pair-recovery попытку с write-set guard только на suggested authored doc + `write_root/shard-pack-manifest.json`; если provider уже записал bootstrap-only authored doc с `ACP_COLLECT_BOOTSTRAP_REPLACE_BEFORE_EXIT`, recovery тоже выбирает pair-recovery, чтобы provider мог переписать markdown + manifest, а не закреплять плохой doc manifest-only repair-ом; если provider уже записал non-bootstrap authored docs, но `shard-pack-manifest.json` missing/invalid, общий engine делает одну manifest-only repair попытку с write-set guard только на `write_root/shard-pack-manifest.json`; ACP не нормализует `documents[].path`, не автозаполняет metadata и не использует semantic stdout repair
