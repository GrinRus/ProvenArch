...
test("qa recovery mock: failed Ask run remains understandable and retryable", async ({ page }) => {
  ...
  const checkout = { id: "service.checkout", name: "Checkout", type: "service", owner_team_id: "team-commerce", confidence: 0.94, provenance_kind: "observed", evidence, path: "model/entities/service.checkout.yaml", available_levels: ["context", "container"], repositories: ["commerce"], related_findings: ["finding.checkout-timeout"], related_questions: [] };
