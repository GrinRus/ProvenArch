...
- generic `codex` plugin/Cloudflare/state-db noise (`chatgpt.com/backend-api/plugins/featured`, Cloudflare HTML, `failed to renew cache TTL`, `state db`, `Operation not permitted`, MCP stream/list 504) учитывать как secondary telemetry; сами по себе такие строки не должны поднимать `runner_unavailable`, если raw runtime/session-summary не зафиксировали terminal provider failure. Default `codex-code` headless invocation and live preflight readiness smoke disable `plugins`, `remote_plugin`, `plugin_sharing`, `apps`, `enable_mcp_apps`, `tool_suggest` и `skill_mcp_dependency_install`, запускаются с `--ignore-user-config`, `--ignore-rules` и isolated auth-only `CODEX_HOME`; runtime/preflight копируют только caller `auth.json` и optional `installation_id`, но не пользовательские `config.toml`, MCP/plugins, app tools, `.tmp/plugins` или rules, чтобы эти поверхности не задерживали artifact-only шаг до первого model action и не давали ложный `operational_host_preflight_failed`.
