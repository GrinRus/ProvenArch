...
func composeQwenToolFirstCollectPromptWithTargets(task acpruntime.Task, validationErr error, docRel string, docTarget string, manifestTarget string, evidencePaths []string) string {
	lines := []string{
		...
		"- Citations: unique ids; repo + concrete file path; non-empty claim_ids; every documents[0].citation_ids value exists in citations[]; every citations[].document_ids value equals documents[0].id. Never reference omitted citations.",
