...
- manifest-only repair prompt содержит task-specific scaffold: фактические authored docs, `artifact_root`, `run_id`, `step_id`, `shard_id`, `repo_scopes`, `path_scopes`, candidate evidence paths и literal JSON skeleton; это не ACP-side manifest synthesis, а provider instruction surface.
