...
5) **Runtime providers (`internal/runtime/*`)** *(implemented baseline)*
   ...
   - collect prompt ставит command-first `FIRST COLLECT ARTIFACT PAIR COMMAND` сразу после provider identity: suggested authored doc path и literal task-specific `shard-pack-manifest.json` skeleton должны быть записаны как focused artifact pair до broad second-pass sweep, а поздний docs-first block только ссылается на этот skeleton и не дублирует heredoc command; `refresh.step1.collect` skeleton сразу содержит policy-minimum `coverage.missing` и stable `questions[]`, чтобы first-action heredoc не конфликтовал с refresh policy; для `init.step1.collect` / `refresh.step1.collect` runtime выполняет максимум одну collect pair-recovery попытку, если provider оставил stdout/stderr diagnostics, но не записал authored artifacts, и максимум одну manifest-only repair попытку, если provider уже записал authored docs в `write_root`, но `shard-pack-manifest.json` отсутствует или невалиден; repair prompts начинаются с command-first absolute heredoc write targets вокруг task-specific skeletons, используют тот же root-file primary evidence preference, instruct overwrite вместо read/diff/patch invalid manifests, не повторяют validation-error field cues, а engine write-set guards разрешают только expected pair или `write_root/shard-pack-manifest.json`
   ...
   - `qwen-code` и `claude-code` policies дополнительно разрешают один fresh retry для missing/invalid artifacts; `qwen-code` также делает первый zero-output `pre_artifact` stall (`stdout=0`, `stderr=0`, no observed artifacts, authored file count `0`) retryable warning, но exhausted silent retry остаётся `runner_unavailable`; `claude-code` сохраняет zero-output fail-fast для non-validator шагов, но на `init|refresh.step3.findings` первый fully silent pre-artifact stall retry-ится как warning; exhausted silent retry остаётся `runner_unavailable`, а partial authored artifacts без валидного manifest/verdict остаются `runtime_contract_failed`
