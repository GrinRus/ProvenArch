...
- frontend init smoke стартует из snapshot/backend `arch-workspace` выбранного backend refresh run: batch harness создает отдельный `frontend-workspace`, merge-ит `snapshots/<refresh-run-id>/reports` поверх copied backend workspace reports, materialize-ит временный `reports/taskruns/run-history.json` для `refresh_run_id` и проверяет Console V2 пользовательский результат через UI/API/artifact inspection: `Source -> Readiness -> Analysis -> Review -> Publish`; canonical frontend smoke не запускает второй provider-backed `init`;
